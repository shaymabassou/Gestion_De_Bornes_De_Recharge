...
export default class EMailNotificationTask implements NotificationTask {
  ...
  private mailerTransport: any;
  private backupMailerTransport: any;
  ...
  public constructor() {
    ...
    if (!Utils.isUndefined(this.emailConfig.smtp)) {
      ...
      this.mailerTransport = nodemailer.createTransport({
        ...
        requireTLS: this.emailConfig.smtp.requireTLS,
        tls: {
    ...
    if (this.emailConfig.disableBackup && !Utils.isUndefined(this.emailConfig.smtpBackup)) {
      ...
      // create nodemailer transporter
      this.backupMailerTransport = nodemailer.createTransport({
        host: this.emailConfig.smtpBackup.host,
        port: this.emailConfig.smtpBackup.port,
        secure: this.emailConfig.smtpBackup.secure,
        requireTLS: this.emailConfig.smtpBackup.requireTLS,
        tls: {
          rejectUnauthorized: false,
        },
        auth: {
          user: this.emailConfig.smtpBackup.user,
          pass: this.emailConfig.smtpBackup.password
        },
        logger: true,
        // debug: true,
      });
  ...
  public async sendNewRegisteredUser(data: NewRegisteredUserNotification, user: User, tenant: Tenant, severity: NotificationSeverity): Promise<NotificationResult> {
    ...
    data.tenantPrimaryColor = (data?.tenantPrimaryColor) ? data?.tenantPrimaryColor : '#7F3F98';
  ...
  private async sendEmail(email: EmailNotificationMessage, data: any, tenant: Tenant, user: User, severity: NotificationSeverity, useSmtpClientBackup = false): Promise<void> {
    ...
    if (this.emailConfig.disableBackup && !this.backupMailerTransport) {
      // No suitable main SMTP server configuration found to send the email
      await Logging.logError({
        tenantID: tenant.id,
        siteID: data?.siteID,
        ...
        action: ServerAction.EMAIL_NOTIFICATION,
        module: MODULE_NAME, method: 'sendEmail',
        message: 'No suitable backup SMTP server configuration found to send email',
        actionOnUser: user
      });
      return;
    }

    if (useSmtpClientBackup && !this.backupMailerTransport) {
    ...
    try {
      // send message
      const messageSent = this.mailerTransport.sendMail(messageToSend, (error, info) => {
        if (error) {
          return console.log('error sending email in mailer transport:', error.message);
        }
      ...
      await Logging.logDebug({
        ...
        module: MODULE_NAME, method: 'sendEmail',
        actionOnUser: user,
        message: `Email Sent: '${rfc2047.decode(messageToSend.subject)}'`,
        detailedMessages: {
          from: rfc2047.decode(messageToSend.from),
          to: rfc2047.decode(messageToSend.to),
          subject: rfc2047.decode(messageToSend.subject),
          content: email.html
        }
      });
    } catch (error) {
      console.log('error sending  email catch: ', error.message);
      try {
        await Logging.logError({
          tenantID: tenant.id ? tenant.id : Constants.DEFAULT_TENANT_ID,
          siteID: data?.siteID,
          ...
          action: ServerAction.EMAIL_NOTIFICATION,
          module: MODULE_NAME, method: 'sendEmail',
          message: `Error Sending Email (${rfc2047.decode(messageToSend.from)}): '${rfc2047.decode(messageToSend.subject)}'`,
          actionOnUser: user,
          detailedMessages: {
            from: rfc2047.decode(messageToSend.from),
            to: rfc2047.decode(messageToSend.to),
  ...
  private async sendEmail_(email: EmailNotificationMessage, data: any, tenant: Tenant, user: User, severity: NotificationSeverity, useSmtpClientBackup = false): Promise<void> {
    // Email configuration sanity checks
  ...
  private async sendSmartEmail(prefix: string, context: Record<string, unknown>, recipient: User, tenant: Tenant, severity: NotificationSeverity, optionalComponents: string[] = []): Promise<EmailNotificationMessage> {
    ...
    const tenantName = (tenant.name === 'default') ? 'SuperAdmin' : tenant.name;
    const emailContent: EmailNotificationMessage = {
      to: recipient.email,
      subject: `WattzHub CPO - ${tenantName} - ${title}`,
      html: html
    };
  ...
  private enrichSourceData(tenant: Tenant, sourceData: any): void {
    ...
    sourceData.wattzhubCpoWebSiteURL = BrandingConstants.WATTZHUB_CPO_URL;
    // Tenant information
    if (tenant.id === Constants.DEFAULT_TENANT_ID) {
      sourceData.tenantName = 'WattzHub CPO'; // TBC - Not sure what to show in emails in that case
    } else {
    ...
    if (this.emailConfig.troubleshootingMode && sourceData.tenantLogoURL.startsWith('http://localhost')) {
      ...
      sourceData.tenantLogoURL = BrandingConstants.WATTZHUB_CPO_URL_LOGO_URL;
    }
  }
