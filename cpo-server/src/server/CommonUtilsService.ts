...
export class CommonUtilsService {
  ...
  private static async isTagIDAuthorizedOnChargingStation(tenant: Tenant, chargingStation: ChargingStation,
    ...
    // Get Authorized Tag
    let tag = await Authorizations.checkAndGetAuthorizedTag(action, tenant, chargingStation, tagID);
    if (!tag) {
      // check for emaid
      const emaid = await Authorizations.checkAndGetAuthorizedEmaid(action,tenant,chargingStation,tagID);
      if(!emaid){
