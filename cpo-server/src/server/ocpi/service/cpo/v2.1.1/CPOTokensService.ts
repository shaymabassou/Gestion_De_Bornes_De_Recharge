...
export default class CPOTokensService {
  ...
  public static async handlePutToken(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    // Get tenant's children if tenant is parent
    let tenants = [tenant];
    ...
    // Update token for parent and children
    for (const childTenant of tenants) {
      await OCPIUtilsService.updateCreateTagWithEmspToken(childTenant, token, tag, tag.user, ServerAction.OCPI_CPO_UPDATE_TOKEN);
    }
