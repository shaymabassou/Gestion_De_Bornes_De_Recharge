...
export default class CPOCommandsService {
  ...
  private static async remoteStartSession(action: ServerAction, req: Request, res: Response,
    ...
    // Get tenant of the station
    const stationTenant = await ChargingStationStorage.getStationTenantByEvseId(tenant, startSession.location_id, startSession.evse_uid);

    if (!stationTenant) {
      await Logging.logError({
        tenantID: stationTenant.id,
        module: MODULE_NAME, method: 'remoteStartSession', action,
        message: `Station Tenant not found, Charging Station with EVSE ID '${startSession.evse_uid}' and Location ID '${startSession.location_id}' does not exist`,
        detailedMessages: { startSession }
      });
    ...
    let localToken = await TagStorage.getTag(
      stationTenant, startSession.token.uid, { withUser: true });
    // Create it on the fly
    if (!localToken) {
      // Check eMSP user
      const emspUser = await OCPIUtils.checkAndCreateEMSPUserFromToken(
        stationTenant, ocpiEndpoint.countryCode, ocpiEndpoint.partyId, startSession.token);
      localToken = await OCPIUtilsService.updateCreateTagWithEmspToken(stationTenant, startSession.token, localToken, emspUser, action);
    }
    ...
    const chargingStation = await ChargingStationStorage.getChargingStationByOcpiLocationEvseUid(
      stationTenant, startSession.location_id, startSession.evse_uid);
    if (!chargingStation) {
      await Logging.logError({
        tenantID: stationTenant.id,
        module: MODULE_NAME, method: 'remoteStartSession', action,
    ...
    // Save Auth
    await ChargingStationStorage.saveChargingStationRemoteAuthorizations(stationTenant, chargingStation.id, chargingStation.remoteAuthorizations);
    ...
    void CPOCommandsService.remoteStartTransaction(action, stationTenant, chargingStation, connector, startSession, ocpiEndpoint);
  ...
  private static async remoteStopSession(action: ServerAction, req: Request, res: Response, next: NextFunction, tenant: Tenant, ocpiEndpoint: OCPIEndpoint): Promise<OCPIResponse> {
    ...
    // Get tenant of the station
    const stationTenant = await TransactionStorage.getTenantBySessionId(tenant, stopSession.session_id);

    const transaction = await TransactionStorage.getOCPITransactionBySessionID(stationTenant, stopSession.session_id);
    if (!transaction) {
      await Logging.logError({
        tenantID: stationTenant.id,
        module: MODULE_NAME, method: 'remoteStopSession', action,
    ...
    const chargingStation = await ChargingStationStorage.getChargingStation(stationTenant, transaction.chargeBoxID);
    if (!chargingStation) {
      await Logging.logError({
        ...LoggingHelper.getTransactionProperties(transaction),
        tenantID: stationTenant.id,
    ...
    // Called Async as the response to the eMSP is sent asynchronously and this request has to finish before the command returns
    void CPOCommandsService.remoteStopTransaction(action, stationTenant, chargingStation, transaction.id, stopSession, ocpiEndpoint);
    return CPOCommandsService.buildOCPIResponse(OCPICommandResponseType.ACCEPTED);
  }
