...
import { OCPIConnector, OCPIConnectorFormat, OCPIConnectorMultipleTarif, OCPIConnectorType, OCPIPowerType, OCPIVoltage } from '../../../types/ocpi/OCPIConnector';
import { OCPIDayOfWeekType, OCPIPriceComponent, OCPIPriceOptions, OCPITariff, OCPITariffDimensionType, OCPITariffElement, OCPITariffRestrictions } from '../../../types/ocpi/OCPITariff';
...
import PricingDefinition, { PricingSource } from '../../../types/Pricing';
...
export default class OCPIUtilsService {
  ...
  public static async getAllCpoLocations(tenants: Tenant[], limit: number, skip: number,
    ...
    for (const tenant of tenants) {
      // Get all sites
      const sites = await SiteStorage.getSitesOCPI(tenant, { issuer: true, public: true }, ['id', 'name', 'address', 'lastChangedOn', 'createdOn']);
      // const sites = await SiteStorage.getSites(tenant,{ issuer: true, public: true }, limit === 0 ? Constants.DB_PARAMS_MAX_LIMIT : { limit, skip },['id', 'name', 'address', 'lastChangedOn', 'createdOn']);
      // Convert Sites to Locations
      ...
      nbrOfSites += sites.count;
      if (nbrOfSites === -1) {
        const sitesCount = await SiteStorage.getSites(tenant, { issuer: true, public: true }, Constants.DB_PARAMS_COUNT_ONLY);
        nbrOfSites += sitesCount.count;
      }
      // Set count
      ocpiLocationsResult.count = nbrOfSites;
    }
    limit = (limit === 0) ? Constants.OCPI_RECORDS_LIMIT : limit;
    ocpiLocationsResult.result = ocpiLocationsResult.result.slice(skip, limit + skip);
    // Return locations
    return ocpiLocationsResult;
  }

  public static async getAllCpoTariffs(tenants: Tenant[], limit: number, skip: number, options: OCPIPriceOptions, ocpiSettings: OcpiSetting, dateFrom?: Date, dateTo?: Date): Promise<DataResult<OCPITariff>> {
    // Result
    const ocpiTariffsResult: DataResult<OCPITariff> = { count: 0, result: [] };
    let nbeOfTariffs = 0;
    let pricingDefinitions: PricingDefinitionDataResult;
    for (const tenant of tenants) {
    //  pricingDefinitions = await PricingStorage.getPricingDefinitionsOCPI(tenant, { dateFrom, dateTo, roaming: true }, ['id', 'dimensions', 'restrictions', 'staticRestrictions', 'name', 'ocpiData', 'description']) as PricingDefinitionDataResult;
       pricingDefinitions = await PricingStorage.getPricingDefinitions(tenant,{ dateFrom, dateTo, roaming: true },{ limit:1000,skip:0 },['id', 'dimensions', 'restrictions', 'staticRestrictions', 'name', 'ocpiData', 'description']) as PricingDefinitionDataResult;

      // Add pricing ocpi data to results
      for (const price of pricingDefinitions.result) {
        price.ocpiData = {
          tariff: await OCPIUtilsService.convertPrice2Tariff(tenant, price, options, ocpiSettings),
          tariffUpdatedOn: new Date(),
          inEmsp: true
        };
        // save ocpiData
        await PricingStorage.savePricingOcpiData(tenant, price.id, price.ocpiData);
        ocpiTariffsResult.result.push(price.ocpiData.tariff);
      }
      nbeOfTariffs += pricingDefinitions.count;
      if (nbeOfTariffs === -1) {
        const pricesCount = await PricingStorage.getPricingDefinitions(tenant, { dateFrom, dateTo }, Constants.DB_PARAMS_COUNT_ONLY);
        nbeOfTariffs += pricesCount.count;
      }
      // Set count
      ocpiTariffsResult.count = nbeOfTariffs;
    }
    // Return locations
    return {
      count: ocpiTariffsResult.count,
      result: ocpiTariffsResult.result.slice(skip,(skip + limit)) ,
    };
  }
  ...
  public static async convertSite2Location(tenant: Tenant, site: Site,
    ...
    return {
      ...
      coordinates: {
        longitude: hasValidSiteGpsCoordinates ? site.address.coordinates[0].toString().slice(0, (site.address.coordinates[0].toString().indexOf('.')) + 7) : Constants.SFDP_LONGITUDE.toString(),
        latitude: hasValidSiteGpsCoordinates ? site.address.coordinates[1].toString().slice(0, (site.address.coordinates[1].toString().indexOf('.')) + 7) : Constants.SFDP_LATTITUDE.toString()
      },
  ...
  public static async convertPrice2Tariff(tenant: Tenant, price: PricingDefinition, options: OCPIPriceOptions, ocpiSetting: OcpiSetting): Promise<OCPITariff> {
    let tariff = {} as OCPITariff;
    tariff.id = (price.ocpiData?.tariff?.id) ? price.ocpiData.tariff.id : RoamingUtils.buildTariffIDWithoutCredentials(price.name);
    tariff.currency = ocpiSetting.currency;
    tariff.tariff_alt_text = [
      {
        language: (ocpiSetting.language) ? ocpiSetting.language : 'FR',
        text: price.description
      }
    ];
    tariff.elements = await this.convertPricing2tariffElements(price);
    tariff.last_updated = new Date();
    return tariff;
  }

  public static async convertPricing2tariffElements(price: PricingDefinition): Promise<OCPITariffElement[]> {
    let tariffElements = [] as OCPITariffElement[];
    let priceComponents = [] as OCPIPriceComponent[];
    let daysOfWeek = [] as OCPIDayOfWeekType[];
    let restrictions = {} as OCPITariffRestrictions;

    // Convert price dimensions according to tariff dimensions types
    if (price.dimensions?.flatFee) {
      priceComponents.push({
        type: OCPITariffDimensionType.FLAT,
        price: price.dimensions.flatFee.price,
        step_size: 1
      });
    }

    if (price.dimensions?.energy) {
      priceComponents.push({
        type: OCPITariffDimensionType.ENERGY,
        price: price.dimensions.energy.price,
        step_size: (price.dimensions.energy.stepSize) ? price.dimensions.energy.stepSize : 1,
      });
    }

    if (price.dimensions?.chargingTime) {
      priceComponents.push({
        type: OCPITariffDimensionType.TIME,
        price: price.dimensions.chargingTime.price,
        step_size: (price.dimensions.chargingTime.stepSize) ? price.dimensions.chargingTime.stepSize : 1,
      });
    }

    if (price.dimensions?.parkingTime) {
      priceComponents.push({
        type: OCPITariffDimensionType.PARKING_TIME,
        price: price.dimensions.parkingTime.price,
        step_size: (price.dimensions.parkingTime.stepSize) ? price.dimensions.parkingTime.stepSize : 1,
      });
    }

    // Convert price restrictions
    if (price.restrictions?.daysOfWeek) {
      for (const restriction of price.restrictions.daysOfWeek) {
        if (restriction === 1) {
          daysOfWeek.push(OCPIDayOfWeekType.MONDAY);
        }
        if (restriction === 2) {
          daysOfWeek.push(OCPIDayOfWeekType.TUESDAY);
        }
        if (restriction === 3) {
          daysOfWeek.push(OCPIDayOfWeekType.WEDNESDAY);
        }
        if (restriction === 4) {
          daysOfWeek.push(OCPIDayOfWeekType.THURSDAY);
        }
        if (restriction === 5) {
          daysOfWeek.push(OCPIDayOfWeekType.FRIDAY);
        }
        if (restriction === 6) {
          daysOfWeek.push(OCPIDayOfWeekType.SATURDAY);
        }
        if (restriction === 7) {
          daysOfWeek.push(OCPIDayOfWeekType.SUNDAY);
        }
      }
      restrictions.day_of_week = daysOfWeek;
    }

    if (price.restrictions?.minEnergyKWh) {
      restrictions.min_kwh = price.restrictions.minEnergyKWh;
    }
    if (price.restrictions?.maxEnergyKWh) {
      restrictions.max_kwh = price.restrictions.maxEnergyKWh;
    }
    if (price.restrictions?.minDurationSecs) {
      restrictions.min_duration = price.restrictions.minDurationSecs;
    }
    if (price.restrictions?.maxDurationSecs) {
      restrictions.max_duration = price.restrictions.maxDurationSecs;
    }

    if (price.restrictions?.timeFrom) {
      restrictions.start_time = price.restrictions.timeFrom;
    }

    if (price.restrictions?.timeTo) {
      restrictions.end_time = price.restrictions.timeTo;
    }

    if (price.staticRestrictions?.validFrom) {
      restrictions.start_date = moment(price.staticRestrictions.validFrom).utc().format('YYYY-MM-DD');
    }

    if (price.staticRestrictions?.validTo) {
      restrictions.end_date = moment(price.staticRestrictions.validTo).utc().format('YYYY-MM-DD');
    }

    // Add price component and restrictions to tariff element
    tariffElements.push({
      ...
      if (!Utils.isEmptyArray(chargingStation.chargePoints)) {
        for (const chargePoint of chargingStation.chargePoints) {
          if (chargePoint.cannotChargeInParallel) {
            chargingStationEvses.push(... await OCPIUtilsService.convertChargingStation2UniqueEvse(tenant, chargingStation, chargePoint, options, settings));
          } else {
            chargingStationEvses.push(... await OCPIUtilsService.convertChargingStation2MultipleEvses(tenant, chargingStation, chargePoint, options, settings));
          }
        }
      } else {
        chargingStationEvses.push(... await OCPIUtilsService.convertChargingStation2MultipleEvses(tenant, chargingStation, null, options, settings));
      }
  ...
  public static async getCpoEvse(tenants: Tenant[], locationId: string, evseUid: string, options: OCPILocationOptions, settings: OcpiSetting): Promise<OCPIEvse> {
    for (const tenant of tenants) {
      // Get site
  ...
  public static async convertConnector2OcpiConnector(tenant: Tenant, chargingStation: ChargingStation,
    connector: Connector, countryID: string, partyID: string, settings: OcpiSetting): Promise<OCPIConnector | OCPIConnectorMultipleTarif> {
    ...
    switch (connector.type) {
      ...
      case ConnectorType.TYPE_1:
        type = OCPIConnectorType.IEC_62196_T1;
        format = OCPIConnectorFormat.CABLE;
        break;
      case ConnectorType.TYPE_1_CCS:
        type = OCPIConnectorType.IEC_62196_T1_COMBO;
        format = OCPIConnectorFormat.CABLE;
        break;
      case ConnectorType.TYPE_3C:
        type = OCPIConnectorType.IEC_62196_T3C;
        format = OCPIConnectorFormat.SOCKET;
        break;
    ...
    const pricingID = OCPIUtilsService.buildConnectorPricingID(tenant, chargingStation, connector, settings);
    const pricingIDs = OCPIUtilsService.buildConnectorPricingIDs(tenant, chargingStation, connector);
    let connectorOCPI = {} as OCPIConnector | OCPIConnectorMultipleTarif;
    connectorOCPI = {
      ...
      tariff_id: await OCPIUtilsService.buildTariffConnectorID(tenant, pricingID),
      tariff_ids: await OCPIUtilsService.buildTariffConnectorIDs(tenant, pricingIDs),
      last_updated: chargingStation.lastSeen
    };

    if (Utils.isEmptyArray(connectorOCPI.tariff_ids)) {
      delete connectorOCPI.tariff_ids;
    }

    return connectorOCPI;
  }
  ...
  private static async convertChargingStation2MultipleEvses(tenant: Tenant, chargingStation: ChargingStation,
    chargePoint: ChargePoint, options: OCPILocationOptions, settings: OcpiSetting): Promise<OCPIEvse[]> {
    ...
    for (const connector of connectors) {
      const evse: OCPIEvse = {
        ...
        connectors: [await OCPIUtilsService.convertConnector2OcpiConnector(
          tenant, chargingStation, connector, options.countryID, options.partyID, settings)],
        last_updated: chargingStation.lastSeen,
        coordinates: {
          longitude: hasValidChargingStationGpsCoordinates ? chargingStation.coordinates[0].toString().slice(0, (chargingStation.coordinates[0].toString().indexOf('.')) + 7) : Constants.SFDP_LONGITUDE.toString(),
          latitude: hasValidChargingStationGpsCoordinates ? chargingStation.coordinates[1].toString().slice(0, (chargingStation.coordinates[1].toString().indexOf('.')) + 7) : Constants.SFDP_LATTITUDE.toString()
        }
      };
  ...
  private static async convertChargingStation2UniqueEvse(tenant: Tenant, chargingStation: ChargingStation,
    ...
    let ocpiConnectors: OCPIConnector[] = [];
    for (const connector of connectors) {
      ocpiConnectors.push(
        await OCPIUtilsService.convertConnector2OcpiConnector(tenant, chargingStation, connector, options.countryID, options.partyID, settings)
      );
    }
  ...
  private static buildConnectorPricingID(tenant: Tenant, chargingStation: ChargingStation, connector: Connector, settings: OcpiSetting): string {
  ...
  private static buildConnectorPricingIDs(tenant: Tenant, chargingStation: ChargingStation, connector: Connector): string[] {
    const defaultTariff = [];
    // Connector?
    if (!Utils.isEmptyArray(connector.tariffIDs)) {
      return connector.tariffIDs;
    }
    // Charging Station?
    if (!Utils.isEmptyArray(chargingStation.tariffIDs)) {
      return chargingStation.tariffIDs;
    }
    // Site Area?
    if (!Utils.isEmptyArray(chargingStation.siteArea?.tariffIDs)) {
      return chargingStation.siteArea.tariffIDs;
    }
    //  Site?
    if (!Utils.isEmptyArray(chargingStation.site?.tariffIDs)) {
      return chargingStation.site.tariffIDs;
    }
    // Default
    return defaultTariff;
  }

  private static async buildTariffConnectorID(tenant: Tenant, pricingId: string): Promise<string> {
    const pricingDefinition = await PricingStorage.getPricingDefinitionByName(tenant, { name: pricingId });
    let tariffID = (pricingDefinition && pricingDefinition.roaming && pricingDefinition.ocpiData?.tariff?.id) ? pricingDefinition.ocpiData?.tariff?.id : pricingId;
    return tariffID;
  }

  private static async buildTariffConnectorIDs(tenant: Tenant, tariffIDs: string[]): Promise<string[]> {
    let tariffOcpiIDs = [] as string[];
    for (const id of tariffIDs) {
      const pricingDefinition = await PricingStorage.getPricingDefinitionByName(tenant, { name: id });
      if (pricingDefinition && pricingDefinition.roaming && pricingDefinition.ocpiData?.tariff?.id) {
        tariffOcpiIDs.push(pricingDefinition.ocpiData?.tariff?.id);
      } else {
        tariffOcpiIDs.push(id);
      }
    }
    return tariffOcpiIDs;
  }
