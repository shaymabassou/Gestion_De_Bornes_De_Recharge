...
export default class SiteAreaService {
  ...
  public static async handleCreateSiteArea(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    // Convert tariff IDs in siteArea to array
    if (await Utils.isTenantRoamingComponentActiveFromToken(req.user, TenantComponents.MULTIPLE_TARIFS)) {
      if (Utils.objectHasProperty(filteredRequest, 'tariffIDs')) {
        tariffIdsArray = (filteredRequest.tariffIDs) ? filteredRequest.tariffIDs.filter((tarifId) => tarifId.trim()) : null;
      }
    }

    const { tariffIDs, ...filteredRequests } = filteredRequest; // Remove tariffIDs from filteredRequest

    // Create Site Area
    const siteArea: SiteArea = {
      ...filteredRequests,
      parentSiteArea: parentSiteArea,
      tariffIDs: tariffIdsArray,
      issuer: true,
  ...
  public static async handleUpdateSiteArea(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (await Utils.isTenantRoamingComponentActiveFromToken(req.user, TenantComponents.MULTIPLE_TARIFS)) {
      if (Utils.objectHasProperty(filteredRequest, 'tariffIDs')) {
        siteArea.tariffIDs = (filteredRequest.tariffIDs) ? filteredRequest.tariffIDs.filter((tarifId) => tarifId.trim()) : null;
      }
    }
