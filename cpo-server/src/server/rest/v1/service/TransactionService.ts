...
export default class TransactionService {
  ...
  public static async handleGetTransactionQr(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    const tenantName = Utils.getTenantFromUrl(req);
    const tenant = await TenantStorage.getTenantBySubdomain(tenantName);
    // Filter
    const filteredRequest = TransactionValidatorRest.getInstance().validateTransactionGetReq(req.query);
    // Check dynamic auth
    const transaction = await TransactionStorage.getTransaction(tenant,filteredRequest.ID);
    res.json(transaction);
    next();
  }
  ...
  public static async handleGetTransactionsToRefund(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    transactions.result.map((transaction) => {
      const pricingDefinitions = transaction?.pricingModel?.pricingDefinitions?.filter((pricingDefinition) => {
      ...
      transaction.tariffData = pricingDefinitions;
      return transaction;
    });
    res.json(transactions);
    next();
  ...
  public static async stopGhostSessions(req : Request, res: Response, next: NextFunction) {
    const transactions = await TransactionStorage.getTransactions(req.tenant, { status:TransactionStatus.ACTIVE } ,{ limit:Constants.DB_RECORD_COUNT_NO_LIMIT,skip:0 });
    const stopedTransactionSIds:number[] = [];
    transactions.result.map(async (transactionStop) => {
      if ((transactionStop.currentTotalDurationSecs > 3600) && (transactionStop.currentTotalConsumptionWh === 0)) {
        try {
          await Logging.logInfo({
            ...LoggingHelper.getTransactionProperties(transactionStop),
            tenantID: req.tenant.id,
            user: req.user, actionOnUser: transactionStop.userID,
            module: MODULE_NAME, method: 'transactionSoftStop',
            message: `${Utils.buildConnectorInfo(transactionStop.connectorId, transactionStop.id)} transaction has been up for an houer whith no consumption is gonna be stoped`, detailedMessages: { transactionStop },
            action: ServerAction.TRANSACTION_SOFT_STOP
          });
          const { transaction, chargingStation, connector } =
        await TransactionService.checkAndGetTransactionChargingStationConnector(ServerAction.TRANSACTION_SOFT_STOP, req.tenant, req.user, transactionStop.id, Action.REMOTE_STOP_TRANSACTION);
          // Soft Stop
          await TransactionService.transactionSoftStop(ServerAction.TRANSACTION_SOFT_STOP, transaction, chargingStation, connector, req, res, next);
          stopedTransactionSIds.push(transactionStop.id);
        } catch (error) {
          await Logging.logError({
            ...
            user: req.user, actionOnUser: transactionStop.userID,
            module: MODULE_NAME, method: 'transactionSoftStop',
            message: `${Utils.buildConnectorInfo(transactionStop.connectorId, transactionStop.id)} transaction has been up for an houer whith no consumption failed to be stoped`, detailedMessages: { transactionStop },
            action: ServerAction.TRANSACTION_SOFT_STOP
          });
    ...
    res.json(stopedTransactionSIds);
    next();
  }
  ...
  private static convertToCSV(req: Request, transactions: Transaction[], writeHeader = true): string {
    ...
    const rows = transactions.map((transaction) => {
      const row = [
        ...
        (transaction.tag) ? (transaction.tag.ocpiToken) ? transaction.tag.ocpiToken.issuer : 'Locale' : 'Locale',
    ...
    const transactions = await TransactionStorage.getTransactions(req.tenant,
      {
        ...
        emsps: filteredRequest.emsp ? filteredRequest.emsp.split('|') : null,
        connectorIDs: filteredRequest.ConnectorID ? filteredRequest.ConnectorID.split('|').map((connectorID) => Utils.convertToInt(connectorID)) : null,
        inactivityStatus: filteredRequest.InactivityStatus ? filteredRequest.InactivityStatus.split('|') : null,
        ocpiAuthorizationID: filteredRequest.authorizationID ? filteredRequest.authorizationID : null,
        ...authorizations.filters
      },
      {
        limit: filteredRequest.Limit,
      ...
      try {
        await Promise.all(
          transactions.result.map(async (transaction) => {
            const results = await this.checkAndGetChargingStationConnector(ServerAction.CHARGING_STATION_CONNECTOR ,req.tenant, req.user,
              transaction.chargeBoxID, transaction.connectorId,Action.READ);

            const connector = results.connector;
        ...
        transactions.result = modifiedTransactions;
        transactions.count = modifiedTransactions.length;
      } catch (error) {
        console.log(error);
      }
    }
