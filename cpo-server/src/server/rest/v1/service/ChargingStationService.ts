...
import ChargingStation, { ChargingStationOcppParameters, ChargingStationQRCode, Command, Connector, ConnectorType, OCPPParams, OcppParameter, StaticLimitAmps } from '../../../../types/ChargingStation';
...
export default class ChargingStationService {
  ...
  public static async handleUpdateChargingStationParams(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    // Update props
    await ChargingStationService.updateChargingStationCommonProperties(action, req.tenant, chargingStation, siteArea, req.user, filteredRequest);
    ...
    // Existing Connectors
    await ChargingStationService.updateChargingStationConnectors(chargingStation, req.user, filteredRequest);
  ...
  public static async handleTriggerSmartChargingByTarif(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    // Check if Component is active
    UtilsService.assertComponentIsActiveFromToken(req.user, TenantComponents.SMART_CHARGING,
      Action.UPDATE, Entity.SITE, MODULE_NAME, 'handleTriggerSmartCharging');
    // Filter
    const filteredRequest = ChargingStationValidatorRest.getInstance().validateSmartChargingByTarifTriggerReq(req.query);
    // Get Site Area
    ...
    if (site) {
      try {
        // Call
        const actionsResponse = await smartCharging.generateTxDefaultProfileByTarif(site.id,chargingStation,filteredRequest.ConnectorID);
        if (!actionsResponse) {
          throw new AppError({
            action, user: req.user,
            errorCode: HTTPError.GENERAL_ERROR,
            module: MODULE_NAME, method: 'handleTriggerSmartChargingByTarif',
            message: 'Error occurred while triggering the smart charging by tarif',
          });
        }
        ...
        await Logging.logInfo({
          tenantID: req.tenant.id,
          action, user: req.user,
          module: MODULE_NAME, method: 'handleSmartCHargingByTaif',
          message: 'error handeling smart cahrging by tarif',
  ...
  public static async handleGenerateQrCodeForConnector(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    const token = Buffer.from(JSON.stringify(chargingStationQRCode)).toString('base64');

    const baseUrl = process.env.NODE_ENV === 'production'
      ? `https://${req.user.tenantSubdomain}.cpo.wattzhub.com/qrcode/`
      : `https://${req.user.tenantSubdomain}.cpo.beta.wattzhub.com/qrcode/`;
    const fullUrl = `${baseUrl}${token}`;

    console.log('ChargingStationService ~ handleGenerateQrCodeForConnector ~ fullUrl:', fullUrl);
    // Generate Charging station QR Code
    const generatedQR = await Utils.generateQrCode(
      Buffer.from(JSON.stringify(chargingStationQRCode)).toString('base64'));

    // Generate Qr code for charing with Qr code feature
    const generatedQRFeature = await Utils.generateQrCode(fullUrl);

    res.json({ image: generatedQR, imageqr: generatedQRFeature, token, fullUrl });
    next();
  }
  ...
  public static async handleGetChargingStation(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    // Add ocpi Connecter Id to charging Station
    ...
    if (Utils.isComponentActiveFromToken(req.user, TenantComponents.BORNE_VIP) && ((req.user.role !== 'A') && (!req.user.rolesACL.includes('siteAdmin')) && (!req.user.rolesACL.includes('siteOwner')))) {
      const modifiedConnectors = [];
      chargingStation.connectors.forEach((connector) => {
        if ((connector.ownerIds === null) || (!connector.ownerIds)) {
          connector.ownerIds = [];
        }
        if ((!connector.isPrivate) || (connector.isPrivate && (connector.ownerIds.includes(req.user.id)))) {
          modifiedConnectors.push(connector);
        }
      });
      chargingStation.connectors = modifiedConnectors;
    }
    res.json(chargingStation);
    next();
  }

  public static async handleGetChargingStationQrCode(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    const tenantName = Utils.getTenantFromUrl(req);
    const tenant = await TenantStorage.getTenantBySubdomain(tenantName);
    // Filter
    const filteredRequest = ChargingStationValidatorRest.getInstance().validateChargingStationQRCodeGetReq({ ...req.params, ...req.query });
    // Check dynamic auth
    const chargingStation = await ChargingStationStorage.getChargingStation(tenant, filteredRequest.ID, {
      withSite: true,
      withSiteArea: true,
    });
  ...
  public static async handleGetChargingStationConnector(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    const filteredRequest = ChargingStationValidatorRest.getInstance().validateChargingStationQRCodeGenerateReq(req.body);
    // Check dynamic auth
    const connectorID = filteredRequest.ConnectorID;
    const chargingStation = await UtilsService.checkAndGetChargingStationAuthorization(req.tenant, req.user,
      filteredRequest.ChargingStationID, Action.GET_CONNECTOR_QR_CODE, action);
    res.json(ChargingStationService.getChargingStationConnector(chargingStation,connectorID));
    next();

  }

  public static async handleGetChargingStationTariff(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    const filteredRequest = ChargingStationValidatorRest.getInstance().validateChargingStationTariffGetReq(req.query);
    // Check dynamic auth
    const chargingStation = await UtilsService.checkAndGetChargingStationAuthorization(req.tenant, req.user,
      filteredRequest.ChargingStationID, Action.GET_CONNECTOR_QR_CODE, action);
    const connector = chargingStation.connectors.find((connectors) => connectors.connectorId === filteredRequest.ConnectorID);
    res.json( await PricingService.getUsedTariff(req.tenant,chargingStation,connector,filteredRequest.Roaming));
    next();

  }

  public static async handleGetChargingStationTariffQr(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    const tenantName = Utils.getTenantFromUrl(req);
    const tenant = await TenantStorage.getTenantBySubdomain(tenantName);
    const filteredRequest = ChargingStationValidatorRest.getInstance().validateChargingStationTariffGetReq(req.query);
    // Check dynamic auth
    const chargingStation = await UtilsService.checkAndGetChargingStationAuthorizationQr(tenant, req.user,
      filteredRequest.ChargingStationID, Action.GET_CONNECTOR_QR_CODE, action);
    const connector = chargingStation.connectors.find((connectors) => connectors.connectorId === filteredRequest.ConnectorID);
    res.json( await PricingService.getUsedTariff(tenant,chargingStation,connector,filteredRequest.Roaming));
    next();

  }

  public static async handleChargingStationConnectorOwner(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    if (!Utils.isComponentActiveFromToken(req.user, TenantComponents.BORNE_VIP)) {
      void Logging.logError({
        tenantID: req.tenant.id,
        user: req.user,
        module: MODULE_NAME, method: 'handleOcppAction', action,
        message: 'VIP CHarging Station control is not active :the Charging Station VIP control function is not ativated , cannot set connector ower',
      });
      res.sendStatus(403);
      return;
    }
    ...
    if (req.body.assign) {
      try {
        chargingStation.connectors.forEach((connector) => {
          if (connector.connectorId === filteredRequest.ConnectorID) {
            if ((connector.ownerIds === null) || (!connector.ownerIds)) {
              connector.ownerIds = [];
            }
            if (!connector.ownerIds.includes(req.body.userID)) {
              connector.ownerIds.push(req.body.userID);
              res.json(Constants.REST_RESPONSE_SUCCESS);
            } else {
              const response = {
                status : '2005',
        ...
        throw new AppError({
          action, user: req.user,
          errorCode: error instanceof AppError ? error.params.errorCode : HTTPError.GENERAL_ERROR,
          module: MODULE_NAME, method: 'assignUserToConnector',
          message: error,
          detailedMessages: { error: error.stack }
        });
      }
    ...
    if (!req.body.assign) {
      try {
        chargingStation.connectors.forEach((connector) => {
          if (connector.connectorId === filteredRequest.ConnectorID) {
            if (connector.ownerIds && connector.ownerIds.length > 0) {
              if (!connector.ownerIds.includes(req.body.userID)) {
                const response = {
                  status : '2005',
                  message :'user already assigned'
                ...
                const index = connector.ownerIds.indexOf(req.body.userID);
                if (index !== -1) {
                  connector.ownerIds.splice(index, 1);
                }
                res.json(Constants.REST_RESPONSE_SUCCESS);
              }
            }
  ...
  private static async getChargingStations(req: Request, filteredRequest: HttpChargingStationsGetRequest,
    ...
    chargingStations.result.map((chargingStation) => {
      if (ocpiSetting?.ocpi?.cpo && chargingStation.connectors.length !== 0) {
        for (let i = 0; i < chargingStation.connectors.length; i++) {
          chargingStation.connectors[i].ocpiConnecterId = RoamingUtils.buildEvseConnectorID(ocpiSetting.ocpi.cpo.countryCode, ocpiSetting.ocpi.cpo.partyID, chargingStation, chargingStation.connectors[i].connectorId);
        }
    ...
    if (Utils.isComponentActiveFromToken(req.user, TenantComponents.BORNE_VIP) && ((req.user.role !== 'A') && (!req.user.rolesACL.includes('siteAdmin')) && (!req.user.rolesACL.includes('siteOwner')))) {
      const modifedCHargingStations = chargingStations.result.map((chargingStation) => {
        const modifiedConnectors = [];
        chargingStation.connectors.forEach((connector) => {
          if ((connector.ownerIds === null) || (!connector.ownerIds)) {
            connector.ownerIds = [];
      ...
      chargingStations.result = modifedCHargingStations.filter((chargingStation) => chargingStation.connectors.length > 0);
    }
  ...
  private static async executeChargingStationStartTransaction(action: ServerAction, chargingStation: ChargingStation, command: Command,
    ...
    // Check Tag/User
    if (tag.issuer && tag.userID !== user.id) {
      throw new BackendError({
        ...LoggingHelper.getChargingStationProperties(chargingStation),
        action,
    ...
    if (Utils.isComponentActiveFromToken(loggedUser, TenantComponents.OCPI)) {
      ...
      try {
        ...
        await Logging.logError({
          ...
          action,
          user: loggedUser,
          message: `Unable to update charging station ${chargingStation.id} in IOP`,
          detailedMessages: { error: error.stack }
        });
      }
    }
  ...
  private static async updateChargingStationCommonProperties(action: ServerAction, tenant: Tenant, chargingStation: ChargingStation,
      siteArea: SiteArea, user: UserToken, filteredRequest: HttpChargingStationParamsUpdateRequest): Promise<void> {
    ...
    if (await Utils.isTenantRoamingComponentActiveFromToken(user, TenantComponents.MULTIPLE_TARIFS)) {
      if (Utils.objectHasProperty(filteredRequest, 'tariffIDs')) {
        chargingStation.tariffIDs = (filteredRequest.tariffIDs) ? filteredRequest.tariffIDs.filter((tarifId) => tarifId.trim()) : null;
      }
    }
  ...
  private static async updateChargingStationConnectors(chargingStation: ChargingStation, user: UserToken,
      filteredRequest: HttpChargingStationParamsUpdateRequest): Promise<void> {
    if (!Utils.isEmptyArray(filteredRequest.connectors)) {
      for (const filteredConnector of filteredRequest.connectors) {
        ...
        if (connector) {
          ...
          if (await Utils.isTenantRoamingComponentActiveFromToken(user, TenantComponents.MULTIPLE_TARIFS)) {
            if (Utils.objectHasProperty(filteredConnector, 'tariffIDs')) {
              connector.tariffIDs = (filteredConnector.tariffIDs) ? filteredConnector.tariffIDs.filter((tarifId) => tarifId.trim()) : null;
            }
          }
        }
        if (Utils.isComponentActiveFromToken(user, TenantComponents.BORNE_VIP)) {
          connector.isPrivate = filteredConnector.isPrivate !== null ? filteredConnector.isPrivate : false;
          connector.ownerIds = filteredConnector.ownerIds !== null ? filteredConnector.ownerIds : [];
        }
      }
    }
  }
  ...
  private static getChargingStationConnector(chargingStation : ChargingStation, connectorID : number) {
