...
const settings: BillingSettings = null;
...
export default class BillingService {
  ...
  // public static async handleGetBillingSettingQr(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
  //   // Check if component is active
  //   UtilsService.assertComponentIsActiveFromToken(QRCODE_ACCESS: UserToken, TenantComponents.BILLING,
  //     Action.READ, Entity.SETTING, MODULE_NAME, 'handleGetBillingSetting');
  //   const billingSettings: BillingSettings = await UtilsService.checkAndGetBillingSettingAuthorization(req.tenant, req.user, null, Action.READ, action);
  //   // Process sensitive data
  //   UtilsService.hashSensitiveData(req.tenant.id, billingSettings);
  ...
  public static async handleActivateAccount(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    void NotificationHandler.sendBillingAccountActivationNotification(
      req.tenant, Utils.generateUUID(), user, { evseDashboardURL: Utils.buildEvseURL(req.tenant.subdomain), user, tenantPrimaryColor:req.tenant?.primaryColor });
    res.json(Object.assign({ id: billingAccount.id }, Constants.REST_RESPONSE_SUCCESS));
    next();
  }
