...
export default class CarService {
  ...
  async handleCreateCarCatalogImg(action: ServerAction, req: Request, res: Response, next: NextFunction) {
    // Filter
    const carsFilePath = 'src/assets/cars/cars.json';

    if (!fs.existsSync(carsFilePath)) {
      console.log('carCatalogsFilePath doesn\'t exist');
    } else {
      console.log('carCatalogsFilePath exists');
    }

    const carCatalogs = JSON.parse(fs.readFileSync(carsFilePath, 'utf8'));
    let vehicleModel;
    let vehicleMake;
    let imagePath;
    let i = 0;
    ...
    for (const carCatalog of carCatalogs) {
      console.log('i:', i);
      vehicleModel = carCatalog.vehicleModel.split(' ').reduce((a, b) => a + '-' + b);
      vehicleMake = carCatalog.vehicleMake.split(' ').reduce((a, b) => a + '-' + b);
      imagePath = `src/assets/cars/img/${carCatalog.id}-${vehicleMake}-${vehicleModel}.jpg`;
      const imageResponse = await axios.get(carCatalog.image, {
        headers: {
      ...
      // Save the file
      await fs.writeFile(imagePath, imageResponse.data, 'base64', (err) => {
        if (err) {
          console.log('add image error', err.message);
          return err;
        }
      });
      carCatalog.imageURLs = [`${carCatalog.id}-${vehicleMake}-${vehicleModel}.jpg`];
      delete carCatalog['image'];
      i++;
    }

    const carCatalogsFile = 'src/assets/cars/car-catalogs-definition.json';
    console.log('******* write file');

    try {
      // Save the file
      await fs.writeFile(carCatalogsFile, JSON.stringify(carCatalogs), (err) => {
        if (err) {
          console.log('add image error', err.message);
          return err;
        }
      ...
      console.log('add carCatalogsFile error', error.message);
    }
  }
