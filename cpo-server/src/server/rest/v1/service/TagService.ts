...
export default class TagService {
  ...
  public static async handleGetTagsEmsps(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    // Filter request
    const filteredRequest = TagValidatorRest.getInstance().validateGetEmsps(req.query);
    // Get authorization filters
    const authorizations = await AuthorizationService.checkAndGetEmspsAuthorizations(
    ...
    let tags = {};
    tags = await TagStorage.getEmspsByTags(req.tenant,
      {
    ...
    const limit = (filteredRequest.Limit) ? Utils.convertToInt(filteredRequest.Limit) : 50;
    tags['result'] = tags['result'].slice(offset, limit + offset);
    res.json(tags);
    next();
  }
  ...
  public static async handleCreateTag(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    // Check if Tag to create belongs to eMsp system, If tenant has double access create tag in local else throw error
    if (filteredRequest.fromEMSP) {
      // Check local tag exists
      if (tag && tag.issuer) {
        throw new AppError({
          ...LoggingHelper.getTagProperties(tag),
          errorCode: HTTPError.TAG_ALREADY_EXIST_ERROR,
          message: `Tag with ID ${filteredRequest.id.toUpperCase()} already exists`,
          module: MODULE_NAME, method: 'handleCreateTag',
          user: req.user,
          action: action
        });
      ...
      // Check parent tenant has double access
      let parentTenant = (req.tenant.type !== 'Child') ? req.tenant : await TenantStorage.getTenant(req.tenant.parentID);

      const doubleAccessSettings = await SettingStorage.getDoubleAccessSettings(parentTenant);
      const hasDoubleAccess = parentTenant.components.doubleAccess.active && doubleAccessSettings?.doubleAccess?.isActive && doubleAccessSettings?.doubleAccess?.emsp;
      if (!hasDoubleAccess) {
        throw new AppAuthError({
          ...
          module: MODULE_NAME, method: 'handleCreateTag',
          value: `Tenant doesn't have access to add eMSP Tag with ID ${filteredRequest.id}`,
        });
      }
      ...
      if (tag && tag.ocpiToken?.issuer !== emspCredentials) {
        throw new AppError({
          ...LoggingHelper.getTagProperties(tag),
          errorCode: HTTPError.TAG_EMSP_DOESNT_BELONG_TO_ORGANISATION_ERROR,
          message: `Tag with ID ${filteredRequest.id} doesn't belong to organisation`,
          module: MODULE_NAME, method: 'handleCreateTag',
          user: req.user,
          action: action
        });
      ...
      newTag = {
        ...
        userID: filteredRequest?.userID,
        default: false,
        visualID: filteredRequest.id.toUpperCase() + '*LOCAL',
        fromEMSP: filteredRequest.fromEMSP
      };
    } else {
      if (tag) {
        throw new AppError({
  ...
  public static async handleUpdateTagByVisualID(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    tag.description = filteredRequest.description;
    tag.default = filteredRequest.default;
    tag.fromEMSP = filteredRequest.fromEMSP;
    tag.lastChangedBy = { id: req.user.id };
    tag.lastChangedOn = new Date();
    // Save
    await TagStorage.saveTag(req.tenant, tag);
  ...
  private static convertToCSV(req: Request, tags: Tag[], writeHeader = true): string {
    ...
    const rows = tags.map((tag) => {
      const row = [
        ...
        tag.visualID,
        tag.description,
        (tag?.fromEMSP) ? tag.fromEMSP : false,
        tag.user?.firstName,
        tag.user?.name,
        tag.user?.email
  ...
  private static async processTag(action: ServerAction, req: Request, importedTag: ImportedTag, tagsToBeImported: ImportedTag[]): Promise<boolean> {
    try {
      ...
      const newImportedTag: ImportedTag = {
        ...
        visualID: (Utils.convertToBoolean(importedTag.fromEMSP)) ? `${importedTag.id.toUpperCase()}*LOCAL` : importedTag.visualID,
