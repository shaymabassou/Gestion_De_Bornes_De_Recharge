...
import Tenant, { TenantComponents, TenantFavicon, TenantLogo } from '../../../../types/Tenant';
...
export default class TenantService {
  ...
  public static async handleGetTenantEmailLogo(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (!logoContent) {
      // A default WattzHub CPO logo - base64 encoded
  ...
  public static async handleGetTenantFavicon(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    const filteredRequest = TenantValidatorRest.getInstance().validateFaviconGetReq(req.query);
    // Get Favicon
    let tenantFavicon: TenantFavicon;
    ...
    //     errorCode: StatusCodes.BAD_REQUEST,
    //     message: 'Tenant must be provided',
    //     module: MODULE_NAME, method: 'handleGetTenantFavicon', action: action,
    //   });
    // }
    // Get the favicon using ID
    if (filteredRequest.ID) {
      const tenant = await TenantStorage.getTenant(filteredRequest.ID);
      if (tenant) {
        tenantFavicon = await TenantStorage.getTenantFavicon(tenant);
      }
      // Get the favicon using Subdomain
    } else if (filteredRequest.Subdomain) {
      const tenant = await TenantStorage.getTenantBySubdomain(filteredRequest.Subdomain, ['id']);
      if (tenant) {
        tenantFavicon = await TenantStorage.getTenantFavicon(tenant);
    ...
    let favicon = tenantFavicon?.favicon;
    if (favicon) {
      // Header
      let header = 'image';
      let encoding: BufferEncoding = 'base64';
  ...
  public static async handleGetTenantColors(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    // Validate
    const filteredRequest = TenantValidatorRest.getInstance().validateColorsGetReq(req.query);
    let colors: any = {
      name: 'WattzHub CPO',
      primaryColor: '#7F3F98',
    ...
    if (filteredRequest.ID) {
      ...
      // Get the colors using Subdomain
    } else if (filteredRequest.Subdomain) {
      const tenant = await TenantStorage.getTenantBySubdomain(filteredRequest.Subdomain, ['id', 'name', 'primaryColor', 'secondaryColor']);
      if (tenant) {
        colors.name = tenant.name;
        colors.primaryColor = tenant.primaryColor;
        colors.secondaryColor = tenant.secondaryColor;
  ...
  public static async handleGetTenant(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    let projectFields = [
      'id', 'name', 'email', 'subdomain', 'type', 'parentID', 'primaryColor', 'secondaryColor', 'components', 'address', 'logo', 'favicon'
    ];
    // Check projection
  ...
  public static async handleTenantOcpi(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    let parentTenant = {} as Tenant;
    let isOcpiActive: boolean;
    let isChildActive = false as boolean;

    // Validate
    const filteredRequest = TenantValidatorRest.getInstance().validateTenantGetReq(req.query);
    UtilsService.assertIdIsProvided(action, filteredRequest.ID, MODULE_NAME, 'handleTenantOcpi', req.user);

    let projectFields = [
      'id', 'name', 'email', 'subdomain', 'type', 'parentID', 'components', 'logo'
    ];
    // Check projection
    const httpProjectFields = UtilsService.httpFilterProjectToArray(filteredRequest.ProjectFields);
    if (!Utils.isEmptyArray(httpProjectFields)) {
    ...
    // If tenant isChid Get parentTenant
    if (tenant.type === 'Child') {
      parentTenant = await TenantStorage.getTenant(tenant.parentID, {}, projectFields);
      isChildActive = parentTenant.components.ocpi?.active;
    }

    // check if tenant isOcpiActive
    isOcpiActive = (tenant.components.ocpi?.active) ? tenant.components.ocpi.active : isChildActive;

    let response = {
      isOcpiActive: isOcpiActive,
      ...tenant
    };

    UtilsService.assertObjectExists(action, tenant, `Tenant ID '${filteredRequest.ID}' does not exist`,
      MODULE_NAME, 'handleTenantOcpi', req.user);
    res.json(response);
    next();
  }

  public static async handleTenantMultipleTarif(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    let parentTenant = {} as Tenant;
    let isMultipleTarifActive: boolean;
    let isChildActive = false as boolean;

    // Validate
    const filteredRequest = TenantValidatorRest.getInstance().validateTenantGetReq(req.query);
    UtilsService.assertIdIsProvided(action, filteredRequest.ID, MODULE_NAME, 'handleTenantMultipleTarif', req.user);
    ...
    if (tenant.type === 'Child') {
      parentTenant = await TenantStorage.getTenant(tenant.parentID, {}, projectFields);
      isChildActive = parentTenant.components.multipleTarifs?.active;
    }

    // check if tenant isOcpiActive
    isMultipleTarifActive = (tenant.components.multipleTarifs?.active) ? tenant.components.multipleTarifs.active : isChildActive;

    let response = {
      isMultipleTarifActive: isMultipleTarifActive,
      ...tenant
    };

    UtilsService.assertObjectExists(action, tenant, `Tenant ID '${filteredRequest.ID}' does not exist`,
      MODULE_NAME, 'handleTenantMultipleTarif', req.user);
    res.json(response);
    next();
  }
  ...
  public static async handleGetTenants(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    // Filter
    let projectFields = [
      'id', 'name', 'email', 'subdomain', 'type', 'parentID', 'primaryColor', 'secondaryColor', 'logo', 'favicon', 'createdOn', 'createdBy', 'lastChangedOn', 'lastChangedBy'
    ];
  ...
  public static async handleGetTenantsList(action: ServerAction,req: Request, res: Response, next: NextFunction): Promise<any> {
    // Validate
    let projectFields = ['name', 'subdomain'];
    const tenants = await TenantStorage.getTenants({},{ limit:100,skip:0 },projectFields);
    // Check auth
    console.log(tenants);
    res.json(tenants);
    next();
  ...
  public static async handleCreateTenant(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    void NotificationHandler.sendNewRegisteredUser(
      ...
      tenantUser,
      {
        ...
        'tenantPrimaryColor': filteredRequest.primaryColor
      }
    );

    //create tenant subdomain
    const tenantDomain = new TenantDomainService(tenant);
    let responseDomain;
    try {
      responseDomain = await tenantDomain.buildTenantDomain(filteredRequest.subdomain);
    } catch (error) {
      console.log('error create tenant', error.message);
      // create error Log
      await Logging.logInfo({
        tenantID: req.tenant.id, user: req.user,
        module: MODULE_NAME, method: 'handleCreateTenant',
        message: `Could not create tenant domaine ${error.message}`,
        action: action,
        detailedMessages: { params: filteredRequest }
      });
      responseDomain = false;
    }
    ...
    res.status(StatusCodes.OK).json(Object.assign({ id: filteredRequest.id, responseDomain: responseDomain ? true : false }, Constants.REST_RESPONSE_SUCCESS));
    next();
  }
  ...
  public static async handleUpdateTenant(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    tenant.type = filteredRequest.type;
    tenant.parentID = filteredRequest.parentID;
    tenant.primaryColor = filteredRequest.primaryColor;
    tenant.secondaryColor = filteredRequest.secondaryColor;
    ...
    if (Utils.objectHasProperty(filteredRequest, 'favicon')) {
      tenant.favicon = filteredRequest.favicon;
    }
    // Update timestamp
    tenant.lastChangedBy = { 'id': req.user.id };
    tenant.lastChangedOn = new Date();
    // Update Tenant
    await TenantStorage.saveTenant(tenant, Utils.objectHasProperty(filteredRequest, 'logo') ? true : false, Utils.objectHasProperty(filteredRequest, 'favicon') ? true : false);
    // OICP
