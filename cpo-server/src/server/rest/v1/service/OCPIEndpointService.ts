...
export default class OCPIEndpointService {
  ...
  public static async handleSetDefaultOcpiEndpoint(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    // Check if component is active
    UtilsService.assertComponentIsActiveFromToken(req.user, TenantComponents.OCPI,
      Action.UPDATE, Entity.OCPI_ENDPOINT, MODULE_NAME, 'handleUpdateOcpiEndpoint');
    // Filter
    const filteredRequest = OCPIEndpointValidatorRest.getInstance().validateOCPIEndpointDefaultSetReq(req.body);
    // Check and get ocpi endpoint
    const ocpiEndpoint = await UtilsService.checkAndGetOCPIEndpointAuthorization(req.tenant, req.user, filteredRequest.id, Action.UPDATE, action);
    // Update timestamp
    ocpiEndpoint.lastChangedBy = { 'id': req.user.id };
    ocpiEndpoint.lastChangedOn = new Date();
    if (Utils.isNullOrUndefined(ocpiEndpoint.default)) {
      ocpiEndpoint.default = false;
    }
    ocpiEndpoint.default = filteredRequest.default;
    // Update OcpiEndpoint
    await OCPIEndpointStorage.saveOcpiEndpoint(req.tenant, ocpiEndpoint);
    await Logging.logInfo({
      tenantID: req.tenant.id,
      user: req.user, module: MODULE_NAME, method: 'handleUpdateOcpiEndpoint',
      message: `Ocpi Endpoint '${ocpiEndpoint.name}' has been updated successfully`,
  ...
  public static async handlePullTokensWithOffsetEndpoint(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    // Check if component is active
    UtilsService.assertComponentIsActiveFromToken(req.user, TenantComponents.OCPI,
    ...
    if (!Utils.isTenantComponentActive(req.tenant, TenantComponents.OCPI)) {
      throw new AppAuthError({
        errorCode: HTTPAuthError.FORBIDDEN,
        user: req.user,
        action: Action.TRIGGER_JOB, entity: Entity.OCPI_ENDPOINT,
    ...
    // Filter
    const filteredRequest = OCPIEndpointValidatorRest.getInstance().validateOCPIEndpointByIdWithOffsetReq(req.body);
    // Get ocpiEndpoint
    ...
    if (!ocpiEndpoint) {
      throw new Error(`Unknown OCPI Endpoint ID '${filteredRequest.id}'`);
    }
    try {
      // Get the OCPI Endpoint
      // Create and Save async task
      await AsyncTaskBuilder.createAndSaveAsyncTasks({
        name: AsyncTasks.OCPI_PULL_TOKENS_WITH_OFFSET,
        action,
        type: AsyncTaskType.TASK,
        tenantID: req.tenant.id,
        parameters: {
          endpointID: filteredRequest.id,
          offset: filteredRequest.offset.toString()
        },
        module: MODULE_NAME,
        method: 'handlePullTokensEndpoint',
      });
  ...
  public static async handlePullInterruptedTokens(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    // Check if component is active
    UtilsService.assertComponentIsActiveFromToken(req.user, TenantComponents.OCPI,
      Action.READ, Entity.OCPI_ENDPOINT, MODULE_NAME, 'handlePullTokensEndpoint');

    // Check if tenant component is active
    if (!Utils.isTenantComponentActive(req.tenant, TenantComponents.OCPI)) {
      throw new AppAuthError({
        errorCode: HTTPAuthError.FORBIDDEN,
        user: req.user,
    ...
    // Filter
    const filteredRequest = OCPIEndpointValidatorRest.getInstance().validateOCPIEndpointByIdReq(req.body);
    // Get ocpiEndpoint
    const ocpiEndpoint = await OCPIEndpointStorage.getOcpiEndpoint(req.tenant, filteredRequest.ID);
    UtilsService.assertObjectExists(action, ocpiEndpoint, `OCPI Endpoint ID '${filteredRequest.ID}' does not exist`,
      MODULE_NAME, 'handlePullTokensEndpoint', req.user);
    try {
      ...
      if (pullTokensLock) {
        try {
          // Get the OCPI Client
          const ocpiClient = await OCPIClientFactory.getCpoOcpiClient(req.tenant, ocpiEndpoint);
          if (!ocpiClient) {
            throw new Error(`OCPI Client not found in Endpoint ID '${filteredRequest.ID}'`);
          }
          await ocpiClient.pullInterruptedTokens();
        } finally {
          // Release the lock
          await LockingManager.release(pullTokensLock);
        }
      ...
      await Logging.logActionExceptionMessage(req.tenant.id, ServerAction.OCPI_CPO_GET_TOKENS, error);
      res.json({ status: HTTPError.GENERAL_ERROR, message: error.message });
    }
  }
  ...
  public static async handlePatchLocationsOcpiEndpoint(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    // Check if component is active
    UtilsService.assertComponentIsActiveFromToken(req.user, TenantComponents.OCPI,
      Action.TRIGGER_JOB, Entity.OCPI_ENDPOINT, MODULE_NAME, 'handlePushEVSEStatusesOcpiEndpoint');
    // Filter
    ...
    const ocpiEndpoint = await UtilsService.checkAndGetOCPIEndpointAuthorization(req.tenant, req.user, filteredRequest.id, Action.TRIGGER_JOB, action);
    // Get the lock
    const patchLocationsLock = await LockingHelper.createOCPIPatchEVSESLock(req.tenant.id, ocpiEndpoint);
    if (!patchLocationsLock) {
      throw new AppError({
        action: ServerAction.OCPI_EMSP_UPDATE_LOCATION,
        errorCode: HTTPError.CANNOT_ACQUIRE_LOCK,
        message: 'Error in pushing the OCPI EVSES: cannot acquire the lock',
        module: MODULE_NAME, method: 'handlePushEVSESOcpiEndpoint',
      });
    }
    try {
      // Create and Save async task
      await AsyncTaskBuilder.createAndSaveAsyncTasks({
        name: AsyncTasks.OCPI_PUSH_LOCATIONS,
        action,
        type: AsyncTaskType.TASK,
        tenantID: req.tenant.id,
        parameters: {
        ...
        module: MODULE_NAME,
        method: 'handlePushEVSESOcpiEndpoint',
      });
    } finally {
      // Release the lock
