...
import User, { ImportedUser, UserRequiredImportProperties, UserRole, UserStatus } from '../../../../types/User';
...
export default class UserService {
    ...
    if (statusHasChanged && req.tenant.id !== Constants.DEFAULT_TENANT_ID) {
      ...
      void NotificationHandler.sendUserAccountStatusChanged(
        ...
        user,
        {
          ...
          'tenantPrimaryColor': req.tenant?.primaryColor
        }
      );
    }
    res.json(Constants.REST_RESPONSE_SUCCESS);
  ...
  public static async handleCreateUser(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    console.log("UserService ~ handleCreateUser ~ authorizations:", authorizations)
    // Get the email
  ...
  public static async handleCreateUserQrCode(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    const tenantName = Utils.getTenantFromUrl(req);
    const tenant = await TenantStorage.getTenantBySubdomain(tenantName);
    // Filter
    const filteredRequest = UserValidatorRest.getInstance().validateUserCreateReq(req.body);
    // Get dynamic auth
    const authorizations = {authorized: true, projectFields: [
      'costCenter',
      'email',
      'firstName',
      'freeAccess',
    ...
    // Create the User
    newUser.id = await UserStorage.saveUser(tenant, newUser, true);
    // Save password
    if (newUser.password) {
      const newPasswordHashed = await Utils.hashPasswordBcrypt(newUser.password);
    ...
    // Update User Admin Data
    await UserService.updateUserAdminData(tenant, newUser, authorizations.projectFields);
    // Assign Site to new User
    await UtilsService.assignCreatedUserToSites(tenant, newUser, authorizations);
    // Update Billing
    await UserService.syncUserAndUpdateBillingDataQr(ServerAction.USER_CREATE, tenant, req.user, newUser);
    // Log
    await Logging.logInfo({
      tenantID: tenant.id,
      user: req.user, actionOnUser: req.user,
  ...
  public static async handleGetUserQr(action: ServerAction, req: Request, res: Response, next: NextFunction): Promise<void> {
    const tenantName = Utils.getTenantFromUrl(req);
    const tenant = await TenantStorage.getTenantBySubdomain(tenantName);
    const user = await UserStorage.getUser(tenant, req.params.id);
    res.json(user);
    next();
  }

  public static async handleGetUserByEmail(action: ServerAction, req: Request, res: Response, email: string, next: NextFunction): Promise<void> {
    const tenantName = await Utils.getTenantFromUrl(req)
    const tenant = await TenantStorage.getTenantBySubdomain(tenantName);
    try {
  ...
  private static async syncUserAndUpdateBillingDataQr(action: ServerAction, tenant: Tenant, loggedUser: UserToken, user: User): Promise<void> {
