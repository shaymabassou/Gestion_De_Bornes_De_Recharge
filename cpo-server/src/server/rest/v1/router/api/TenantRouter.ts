...
export default class TenantRouter {
  ...
  public buildRoutes(): express.Router {
    ...
    this.buildRouteTenantOcpi();
    this.buildRouteTenantMultipleTarifs();
    return this.router;
  }
  ...
  private buildRouteTenantOcpi(): void {
    this.router.get(`/${RESTServerRoute.REST_TENANT_OCPI}`, (req: Request, res: Response, next: NextFunction) => {
      req.query.ID = sanitize(req.params.id);
      void RouterUtils.handleRestServerAction(TenantService.handleTenantOcpi.bind(this), ServerAction.TENANT, req, res, next);
    });
  }

  private buildRouteTenantMultipleTarifs(): void {
    this.router.get(`/${RESTServerRoute.REST_TENANT_MULTIPLE_TARIFS}`, (req: Request, res: Response, next: NextFunction) => {
      req.query.ID = sanitize(req.params.id);
      void RouterUtils.handleRestServerAction(TenantService.handleTenantMultipleTarif.bind(this), ServerAction.TENANT, req, res, next);
    });
  }
