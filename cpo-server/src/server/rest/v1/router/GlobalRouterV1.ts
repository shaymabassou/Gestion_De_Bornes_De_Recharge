...
import EmaidRouter from './api/EmaidRouter';
import CertificateRouter from './api/CertificateRouter';
...
export default class GlobalRouterV1 {
  ...
  protected buildRouteAPI(): void {
    this.router.use('/api',
      ...
      [
        ...
        new EmaidRouter().buildRoutes(),
        new CertificateRouter().buildRoutes(),
  ...
  protected buildEditRouteDocs(): void {
    this.router.use('/edit', new SwaggerRouter().buildRoutes(),new PricingRouter().buildEditRoute());
  }
}
