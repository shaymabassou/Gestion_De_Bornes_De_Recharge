  ...
  public static async processStartTransaction(tenant: Tenant, transaction: Transaction, chargingStation: ChargingStation, siteArea: SiteArea, user: User): Promise<void> {
    ...
    if (billingImpl) {
      try {
        ...
        if (withBillingActive) {
          const paymentMethods = await billingImpl.getPaymentMethods(user);
          let expired = true;
          const thisDate = new Date();
          paymentMethods.map((paymentMethod)=>{
            if (paymentMethod.expiringOn > thisDate) {
              expired = false;
            }
          });
          if (expired === true) {
            throw new BackendError({
              ...
              message:'all user payment methods ae expired please add a new method'
            });
          }
        }
        // Update Billing Data
