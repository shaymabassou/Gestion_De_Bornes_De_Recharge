...
export default class StripeBillingIntegration extends BillingIntegration {
  ...
  public async checkConnection(): Promise<void> {
    ...
    if (!this.stripe) {
      try {
        //console.log("publicKey",this.settings.stripe.publicKey);
        ...
        console.log("checkConnection error", error.message);
      ...
      try {
        ...
        //console.log("this.productionMode",this.productionMode);
        //console.log('Utils.isProductionEnv()',Utils.isProductionEnv());
  ...
  public async synchronizeUnpaidBills(transaction: Transaction, invoice: BillingInvoice): Promise<BillingDataTransactionStop> {
    await this.checkConnection();
    // Check object
    this.checkBillTransaction(transaction);
    try {
      let stripeInvoice = await this.getStripeInvoice(invoice.invoiceID);
      let billingInvoice: BillingInvoice = await BillingStorage.getInvoiceByInvoiceID(this.tenant, stripeInvoice.id);
      const accountData = await this.retrieveAccountData(transaction);
      const invoiceItem = this.convertToBillingInvoiceItem(transaction,accountData);
      // If stripeInvoice is paid synchronize invoice else resend email notification
      ...
      await Logging.logError({
        ...
        module: MODULE_NAME, method: 'synchronizeUnpaidBills',
        message: `Failed to synchronize unpaid transaction - Transaction ID '${transaction.id}'`,
        detailedMessages: { error: error.stack },
      });
    }
    return {
