...
export default class Authorizations {
  ...
  public static async canUpdateTransaction(loggedUser: UserToken, transaction: Transaction): Promise<boolean> {
    ...
    const context: AuthorizationContext = {
      ...
      tagIDs: loggedUser.tagIDs,
      tagID: transaction.tagID,
      // emaidIDs: loggedUser.emaidIDs,
      // emaidID: transaction.emaidID,
      site: transaction.siteID,
      sites: loggedUser.sites,
      sitesAdmin: loggedUser.sitesAdmin,
      sitesOwner: loggedUser.sitesOwner
  ...
  public static async canListEmaids(loggedUser: UserToken): Promise<boolean> {
    return Authorizations.canPerformAction(loggedUser, Entity.EMAID, Action.LIST);
  }

  public static async canReadEmaid(loggedUser: UserToken): Promise<boolean> {
    return Authorizations.canPerformAction(loggedUser, Entity.EMAID, Action.READ);
  }

  public static async canDeleteEmaid(loggedUser: UserToken): Promise<boolean> {
    return Authorizations.canPerformAction(loggedUser, Entity.EMAID, Action.DELETE);
  }

  public static async canCreateEmaid(loggedUser: UserToken): Promise<boolean> {
    return Authorizations.canPerformAction(loggedUser, Entity.EMAID, Action.CREATE);
  }

  public static async canUpdateEmaid(loggedUser: UserToken): Promise<boolean> {
    return Authorizations.canPerformAction(loggedUser, Entity.EMAID, Action.UPDATE);
  }

  public static async canImportEmaids(loggedUser: UserToken, authContext?: AuthorizationContext): Promise<AuthorizationResult> {
    return Authorizations.can(loggedUser, Entity.EMAID, Action.IMPORT, authContext);
  }

  public static async canExportEmaids(loggedUser: UserToken, authContext?: AuthorizationContext): Promise<AuthorizationResult> {
    return Authorizations.can(loggedUser, Entity.EMAID, Action.EXPORT, authContext);
  }
...
//POUR LES CERTIFICATES
  public static async canListCertificates(loggedUser: UserToken): Promise<boolean> {
    return Authorizations.canPerformAction(loggedUser, Entity.CERTIFICATE, Action.LIST);
  }

  public static async canReadCertificate(loggedUser: UserToken): Promise<boolean> {
    return Authorizations.canPerformAction(loggedUser, Entity.CERTIFICATE, Action.READ);
  }

  public static async canDeleteCertificate(loggedUser: UserToken): Promise<boolean> {
    return Authorizations.canPerformAction(loggedUser, Entity.CERTIFICATE, Action.DELETE);
  }
  ...
  public static isQrCode(user: UserToken | User): boolean {
    return user.role === UserRole.QRCODE;
  }
  ...
  public static async checkOCPIAuthorizedUser(action: ServerAction, tenant: Tenant, chargingStation: ChargingStation,
    ...
    const ocpiClient = await OCPIClientFactory.getAvailableOcpiClientModified(parentTenant, OCPIRole.CPO,tag.ocpiToken.issuer) as CpoOCPIClient;
    if (!ocpiClient) {
      throw new BackendError({
        user: user, action,
        module: MODULE_NAME, method: 'checkOCPIAuthorizedUser',
    ...
    switch (authAction) {
      ...
      case Action.AUTHORIZE:
        ...
        user.authorizationID = await Authorizations.checkAndGetOCPIAuthorizationIDFromIOPRemoteStartTransaction(
          action, parentTenant, chargingStation, connector, tag, transaction);
  ...
  public static async checkAndGetAuthorizedEmaid(action: ServerAction, tenant: Tenant, chargingStation: ChargingStation, emaidID: string): Promise<Emaid> {
    // Get Emaid
      const emaid = await EmaidStorage.getEmaid(tenant, emaidID, { withUser: true });
      if (emaid) {
        // Inactive Emaid
        if (!emaid.active) {
          throw new BackendError({
            ...LoggingHelper.getChargingStationProperties(chargingStation),
            action: action,
            message: `Emaid ID '${emaidID}' is not active`,
            module: MODULE_NAME, method: 'checkAndGetAuthorizedEmaid',
            user: emaid.user,
            detailedMessages: { emaid }
          });
        }
        // No User
        if (!emaid.user) {
          throw new BackendError({
            ...LoggingHelper.getChargingStationProperties(chargingStation),
            action: action,
            message: `Emaid ID '${emaidID}' is not assigned to a User`,
            module: MODULE_NAME, method: 'checkAndGetAuthorizedTag',
            user: emaid.user,
            detailedMessages: { emaid }
          });
    ...
    public static async checkAndGetAuthorizedCertificate(action: ServerAction, tenant: Tenant, chargingStation: ChargingStation, certificateID: string): Promise<Certificate> {
      // Get Certificate
        const certificate = await CertificateStorage.getCertificate(tenant, certificateID);
        if (certificate) {
          ...
          if (!certificate.status) {
            throw new BackendError({
              ...LoggingHelper.getChargingStationProperties(chargingStation),
              action: action,
              message: `Certificate ID '${certificateID}' is not active`,
              module: MODULE_NAME, method: 'checkAndGetAuthorizedCertificate',
              detailedMessages: { certificate }
            });
          }
