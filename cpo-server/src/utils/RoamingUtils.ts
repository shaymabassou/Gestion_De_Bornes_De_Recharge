...
export default class RoamingUtils {
  ...
  public static buildTariffID(countryCode: string, partyId: string, name:string): string {
    const id = Math.round(Math.random() * 10000);
    const tariffID = `${RoamingUtils.buildOperatorName(countryCode, partyId)}*${name.toUpperCase()}-${id}`;
    return tariffID.replace(/[\W_]+/g, '*').toUpperCase();
  }

  public static buildTariffIDWithoutCredentials(name:string): string {
    const id = Math.round(Math.random() * 10000);
    const tariffID = `${name.toUpperCase()}-${id}`;
    return tariffID.replace(/[\W_]+/g, '*').toUpperCase();
  }
