...
import { AnalyticsSettingsType, AssetSettingsType, BillingSettingsType, CarConnectorSettingsType, CryptoKeyProperties, DoubleAccessSettingsType, MultipleTarifsSettingsType, PricingSettingsType, RefundSettingsType, RoamingSettingsType, SettingDBContent, SmartChargingContentType } from '../types/Setting';
...
export default class Utils {
  ...
  public static generateEmaidID(size = 8): string {
    return [...Array(size)].map(() => Math.floor(Math.random() * 16).toString(16)).join('').toUpperCase();
  }
  ...
  public static getTenantFromUrl(req: Request): string {
    const url = req.headers.origin || req.headers.referer;

    if (!url) {
      throw new Error('Origin or Referer header not found');
    }
  ...
  public static async isTenantRoamingComponentActiveFromToken(userToken: UserToken, componentName: TenantComponents): Promise<boolean> {
    // Get tenant from user Token
    const tenant = await TenantStorage.getTenant(userToken.tenantID, {},
      [
        'id', 'name', 'email', 'subdomain', 'type', 'parentID', 'components', 'logo'
      ]);

    const parentTenant = (tenant.type === 'Child') ? await TenantStorage.getTenant(tenant.parentID, {}, ['id', 'name', 'email', 'subdomain', 'type', 'parentID', 'components', 'logo'])
      : tenant;

    // Check component active
    if (typeof parentTenant.components[`${componentName}`] === 'undefined') {
      return false;
    }
    if (typeof parentTenant.components[`${componentName}`] === null) {
      return false;
    }
    let isComponentActive = (parentTenant.components[`${componentName}`].active) ? parentTenant.components[`${componentName}`].active : tenant.components[`${componentName}`].active;
    return isComponentActive;
  }

  public static async isTenantRoamingComponentActive(tenant: Tenant, componentName: TenantComponents): Promise<boolean> {
    // Get tenant from user Token
    const parentTenant = (tenant.type === 'Child') ? await TenantStorage.getTenant(tenant.parentID, {}, ['id', 'name', 'email', 'subdomain', 'type', 'parentID', 'components', 'logo'])
      : tenant;
  ...
  public static getChargePointFromConnectorID(chargingStation: ChargingStation, conectorID: number): ChargePoint {
    if (!chargingStation.chargePoints) {
      return null;
    }
    return chargingStation.chargePoints.find((chargePoint) => chargePoint && (chargePoint.connectorIDs.includes(conectorID)));
  }
  ...
  public static buildRestServerTenantEmailLogoURL(tenantID: string): string {
    if (!tenantID || tenantID === Constants.DEFAULT_TENANT_ID) {
      ...
      return `${Utils.buildRestServerURL(true)}/v1/util/tenants/email-logo?ts=` + new Date().getTime();
    }
    // URL to the tenant logo (if any) or the Open -e-mobility logo as a fallback
    return `${Utils.buildRestServerURL(true)}/v1/util/tenants/email-logo?ID=${tenantID}&ts=` + new Date().getTime();
  }
  ...
  public static createDefaultSettingContent(componentName: string, activeComponentContent: TenantComponentContent, currentSettingContent: SettingDBContent): SettingDBContent {
    switch (componentName) {
      ...
      case TenantComponents.CAR_CONNECTOR:
        ...
        break;
      // double Access
      case TenantComponents.DOUBLE_ACCESS:
        if (!currentSettingContent) {
          // Only Double Access
          return {
            'type': DoubleAccessSettingsType.DOUBLE_ACCESS,
            'doubleAccess': {}
          } as SettingDBContent;
        }
        break;
      // Multiple Tarifs
      case TenantComponents.MULTIPLE_TARIFS:
        if (!currentSettingContent) {
          // Only tenants eMsp and CPO
          return {
            'type': MultipleTarifsSettingsType.MULTIPLE_TARIFS,
            'isActive': activeComponentContent.active
          } as SettingDBContent;
        }
        break;
    }
  ...
  public static getBorneVipAllowedChargingStations(chargingStations : ChargingStation[]) {
    const modifedCHargingStations = chargingStations.map((chargingStation) => {
      const modifiedConnectors = chargingStation.connectors.filter((connector) => (!connector.isPrivate || (connector.isPrivate)));
      return {
        ...chargingStation,
        connectors: modifiedConnectors,
      };
    });
    chargingStations = modifedCHargingStations.filter((chargingStation) => chargingStation.connectors.length > 0);
    return chargingStations;
  }

  public static isUserAuthorizedToSeeConnector(chargingStation : ChargingStation , connectorID : number , user : UserToken) {
    if (user.role === UserRole.ADMIN || user.sitesAdmin || user.sitesOwner) {
      return true;
    } else if (user.role === UserRole.BASIC || user.sitesAdmin || user.sitesOwner) {
      const connector = this.getConnectorFromID(chargingStation,connectorID);
      if (connector.ownerIds.find((ownerID) => ownerID === user.id)) {
  ...
  public static sortPricingDefitionsByRestrection(pricing:PricingDefinition[]) {
    return pricing.sort((a, b) => {
      if ((!a.restrictions?.timeFrom || !a.restrictions?.timeTo) && (b.restrictions?.timeFrom && b.restrictions?.timeTo)) {
        return 1;
      } else if ((a.restrictions?.timeFrom && a.restrictions?.timeTo) && (!b.restrictions?.timeFrom || !b.restrictions?.timeTo)) {
        return -1;
      } else if ((!a.restrictions?.timeFrom || !a.restrictions?.timeTo) && (!b.restrictions?.timeFrom || !b.restrictions?.timeTo)) {
        return 0;
      } else if (a.restrictions.timeFrom && a.restrictions.timeTo && b.restrictions.timeFrom && b.restrictions.timeTo) {
  ...
  public static convertString2TimeFromMidnight(time:string) {
