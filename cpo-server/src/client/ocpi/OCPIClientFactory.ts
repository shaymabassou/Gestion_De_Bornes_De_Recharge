...
export default class OCPIClientFactory {
  ...
  public static async getAvailableOcpiClientModified(tenant: Tenant, ocpiRole: OCPIRole ,tokenIsuuser: string): Promise<CpoOCPIClient|EmspOCPIClient> {
    const ocpiEndpoints = await OCPIEndpointStorage.getOcpiEndpoints(tenant, { role: ocpiRole }, Constants.DB_PARAMS_MAX_LIMIT);
    const issuerString = tokenIsuuser.split('*');
    console.log('--------------------------issuer string: ',issuerString);
    let specificEndpoint = ocpiEndpoints.result.filter((endpoint) => (((endpoint.countryCode === issuerString[0]) && (endpoint.partyId === issuerString[1])|| (endpoint.businessDetails.name === issuerString[0]))));
    if (!Utils.isEmptyArray(specificEndpoint)) {
      console.log('-------------------------Jai trouvé la handshake  desirée c est:',specificEndpoint[0].name);
    } else {
      specificEndpoint = ocpiEndpoints.result.filter((endpoint) => (endpoint?.default === true));
      if (!Utils.isEmptyArray(specificEndpoint)) {
        console.log('-----------------------------found a default handshake:',specificEndpoint[0].name);
      }
    }
    const endpointsTouse = (!Utils.isEmptyArray(specificEndpoint)) ? specificEndpoint : ocpiEndpoints.result;
    for (const ocpiEndpoint of endpointsTouse) {
      if (ocpiEndpoint.status === OCPIRegistrationStatus.REGISTERED) {
        console.log('-------------------------ocpiEndpoint.name:',ocpiEndpoint.name);
