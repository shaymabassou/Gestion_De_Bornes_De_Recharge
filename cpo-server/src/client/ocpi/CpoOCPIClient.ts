...
export default class CpoOCPIClient extends OCPIClient {
  ...
  public async pullTokens(partial = false): Promise<OCPIResult> {
    ...
    do {
      ...
      try {
        ...
        this.ocpiEndpoint.lastCPOPullTokenError = {
          dateTime: new Date(),
      ...
      if (!response?.data?.data) {
        ...
        do {
          await Logging.logDebug({
            ...
            message: `pull token failed , trying for the ${restReties} from ${tokensUrl}`,
            module: MODULE_NAME, method: 'pullTokens'
          });
          try {
            const responseRetrie = await this.axiosInstance.get(
              tokensUrl,
              {
                headers: {
                  Authorization: `Token ${this.ocpiEndpoint.token}`
            ...
            if (responseRetrie.data.data) {
              await Logging.logDebug({
                tenantID: this.tenant.id,
                action: ServerAction.OCPI_CPO_GET_TOKENS,
                message: `pull token succeded after ${restReties} try from ${tokensUrl}`,
                module: MODULE_NAME, method: 'pullTokens'
              });
              response = responseRetrie ;
              break;
            } else {
              restReties += 1 ;
              await Logging.logDebug({
                tenantID: this.tenant.id,
                action: ServerAction.OCPI_CPO_GET_TOKENS,
                message: `pull token ${restReties} try on ${tokensUrl}  failed trying again after 1 min `,
                module: MODULE_NAME, method: 'pullTokens'
              });
              await this.sleep(60000);
            }
          } catch (error) {
            restReties = restReties + 1 ;
            await this.sleep(60000);
            await Logging.logDebug({
              tenantID: this.tenant.id,
              action: ServerAction.OCPI_CPO_GET_TOKENS,
              message: `pull token failed when  trying for the ${restReties} from ${tokensUrl} with error : ${error}`,
              module: MODULE_NAME, method: 'pullTokens'
            });
          }
        } while ((restReties <= 99) && (!response?.data?.data));
        if (!response.data.data) {
          throw new BackendError({
            action: ServerAction.OCPI_CPO_GET_TOKENS,
            message: 'Invalid response from Pull tokens',
            module: MODULE_NAME, method: 'pullTokens',
            detailedMessages: { data: tokensUrl }
          });
        }
      }
      ...
      if (this.tenant.type === 'Parent') {
        const children = await TenantStorage.getTenants({ tenantParentID: this.tenant.id }, Constants.DB_PARAMS_MAX_LIMIT);
      ...
      // Store tokens for in every tenant's db
      for (const tenant of tenants) {

        // Get all the EMSP Users
        const myEmspUsersMap = new Map<string, User>();
        const myEmspUsers = (await UserStorage.getUsers(tenant, { issuer: false }, Constants.DB_PARAMS_MAX_LIMIT)).result;
        for (const emspUser of myEmspUsers) {
          myEmspUsersMap.set(emspUser.email, emspUser);
        }
        ...
        if (!Utils.isEmptyArray(tokens)) {
          ...
          await this.checkAndCreateEMSPUsersFromTokens(tokens, myEmspUsersMap, tenant);
  ...
  public async pullTokensWithOffSet(partial = false , offset:number): Promise<OCPIResult> {
    const result: OCPIResult = {
      success: 0,
      failure: 0,
      total: 0,
    ...
    if (partial) {
      ...
      // // Update URL
      momentFrom = moment().utc().subtract(1, 'hours').startOf('hour');
      tokensUrl = `${tokensUrl}?date_from=${momentFrom.format()}&offset=${offset}&limit=1000`;
    } else {
      // Update URL
      tokensUrl = `${tokensUrl}?offset=${offset}&limit=1000`;
    }
    // Get all the EMSP Users
    const emspUsersMap = new Map<string, User>();
    const emspUsers = (await UserStorage.getUsers(this.tenant, { issuer: false }, Constants.DB_PARAMS_MAX_LIMIT)).result;
  ...
  public async pullInterruptedTokens(): Promise<OCPIResult> {
    const result: OCPIResult = {
      success: 0,
      failure: 0,
      total: 0,
    ...
    // Get tokens endpoint url
    if (this.ocpiEndpoint?.lastCPOPullTokenError?.url) {
      tokensUrl = this.ocpiEndpoint.lastCPOPullTokenError.url;
    } else {
      tokensUrl = `${this.getEndpointUrl('tokens', ServerAction.OCPI_CPO_GET_TOKENS)}?limit=1000`;
    }
    ...
    do {
      const startTimeLoop = new Date().getTime();
      console.log('tokensUrl', tokensUrl, 'totalNumberOfToken', totalNumberOfToken);
      console.time('-- Call IOP');
      let response;
      try {
        // Call IOP
        response = await this.axiosInstance.get(
      ...
      if (!response?.data?.data) {
        ...
        do {
          await Logging.logDebug({
            tenantID: this.tenant.id,
            action: ServerAction.OCPI_CPO_GET_TOKENS,
            message: `pull token failes trying for the ${restReties}   from ${tokensUrl}`,
            module: MODULE_NAME, method: 'pullTokens'
          });
          const responseRetrie = await this.axiosInstance.get(
            tokensUrl,
          ...
          if (responseRetrie.data.data) {
            ...
            await Logging.logDebug({
              tenantID: this.tenant.id,
              action: ServerAction.OCPI_CPO_GET_TOKENS,
              message: `pull token ${restReties} on ${tokensUrl} try failed trying again after 1 min `,
              module: MODULE_NAME, method: 'pullTokens'
            });
            await this.sleep(60000);
          }
      ...
      for (const tenant of tenants) {
        // Get all the EMSP Users
        const emspUsersMap = new Map<string, User>();
        const emspUsers = (await UserStorage.getUsers(tenant, { issuer: false }, Constants.DB_PARAMS_MAX_LIMIT)).result;
        for (const emspUser of emspUsers) {
          emspUsersMap.set(emspUser.email, emspUser);
        }
        // Get all Tags at once from the DB
        ...
        if (!Utils.isEmptyArray(tokens)) {
          // Check and get eMSP users from Tokens
          await this.checkAndCreateEMSPUsersFromTokens(tokens, emspUsersMap, tenant);
          // Update the tags
          await Promise.map(tokens, async (token) => {
            try {
              // Get eMSP user
    ...
    await Logging.logOcpiResult(this.tenant.id, ServerAction.OCPI_CPO_GET_TOKENS,
      MODULE_NAME, 'pullTokens', result,
      `{{inSuccess}} token(s) were successfully pulled in ${executionDurationSecs}s ${momentFrom.format()}`,
      `{{inError}} token(s) failed to be pulled in ${executionDurationSecs}s ${momentFrom.format()}`,
      `{{inSuccess}} token(s) were successfully pulled and {{inError}} failed to be pulled in ${executionDurationSecs}s ${momentFrom.format()}`,
      `No tokens have been pulled ${momentFrom.format()}`
    );
    return result;
  }
  ...
  public async startSession(ocpiToken: OCPIToken, chargingStation: ChargingStation, transaction: Transaction, tenant: Tenant): Promise<void> {
    // Get tokens endpoint url
    ...
    const ocpiLocation: OCPILocation = await this.convertChargingStationToOCPILocation(tenant, site, chargingStation,
  ...
  public async updateSession(transaction: Transaction, childTenant: Tenant): Promise<void> {
    ...
    transaction.ocpiData.session.charging_periods = await this.buildChargingPeriods(childTenant, transaction);
  ...
  public async stopSession(transaction: Transaction, tenant: Tenant): Promise<void> {
    if (!transaction.ocpiData?.session) {
      throw new BackendError({
        ...LoggingHelper.getTransactionProperties(transaction),
    ...
    transaction.ocpiData.session.charging_periods = await this.buildChargingPeriods(tenant, transaction);
  ...
  public async postCdr(transaction: Transaction, tenant: Tenant): Promise<boolean> {
    if (!transaction.stop) {
      throw new BackendError({
        ...LoggingHelper.getTransactionProperties(transaction),
    ...
    let tariffs = [] as OCPITariff[];
    tariffs = await Promise.all(transaction?.pricingModel?.pricingDefinitions?.filter((pricingDefinition) => {
    ...
    }).flatMap(async (pricedTariff) => {
      const ocpiTariff = await PricingStorage.getPricingDefinition(tenant, pricedTariff.id, {}, ['ocpiData.tariff']);
      if (!Utils.isEmptyObject(ocpiTariff)) {
        return ocpiTariff.ocpiData.tariff;
      }
    })).filter((tarif) => !Utils.isNullOrUndefined(tarif));
    ...
    transaction.ocpiData.cdr = {
      ...
      charging_periods: await this.buildChargingPeriods(tenant, transaction),
    ...
    if (!Utils.isEmptyArray(transaction.ocpiData.cdr.charging_periods) &&
      ...
      try {
        ...
        await Logging.logError({
          ...
          tenantID: tenant.id,
          action: ServerAction.OCPI_CPO_PUSH_CDRS,
          message: `${Utils.buildConnectorInfo(transaction.connectorId, transaction.id)} OCPI CDR hasn't been sent `,
          module: MODULE_NAME, method: 'postCdr',
          detailedMessages: ('IOP server not available : ' + error)
        });
      }
      return true;
    }
  ...
  public async putTariff(pricing: PricingDefinition): Promise<AxiosResponse<any>> {
    const tariffId = pricing.ocpiData?.tariff?.id;
    // Check for input parameter
    if (!tariffId) {
      throw new BackendError({
        ...LoggingHelper.getPricing(pricing),
        action: ServerAction.OCPI_CPO_UPDATE_STATUS,
        module: MODULE_NAME, method: 'putTariff',
        message: 'Invalid parameters',
      });
    }
    // Get tariffs endpoint url
    const tariffsUrl = this.getEndpointUrl('tariffs', ServerAction.OCPI_CPO_UPDATE_STATUS);
    ...
    // Build url to EVSE
    const fullUrl = tariffsUrl + `/${countryCode}/${partyID}/${tariffId}`;
    // Build payload
    const newTariff = pricing.ocpiData.tariff;
    // Call IOP
    const response = await this.axiosInstance.put(
      fullUrl,
      newTariff,
    ...
    await Logging.logInfo({
      ...LoggingHelper.getPricing(pricing),
      tenantID: this.tenant.id,
      action: ServerAction.OCPI_CPO_UPDATE_TARIFF,
      message: `OCPI Tariff ID '${tariffId}' has been Inserted successfully to IOP`,
      module: MODULE_NAME, method: 'putTariff',
      detailedMessages: { tariffID: tariffId, newTariff, response: response.data }
    });
    return response;
  }

  public async patchTariff(pricing: PricingDefinition): Promise<AxiosResponse<any>> {
    const tariffId = pricing.ocpiData?.tariff?.id;
    // Check for input parameter
    if (!tariffId) {
      throw new BackendError({
        ...LoggingHelper.getPricing(pricing),
        action: ServerAction.OCPI_CPO_UPDATE_STATUS,
        module: MODULE_NAME, method: 'patchTariff',
        message: 'Invalid parameters',
      });
    }
    // Get tariffs endpoint url
    ...
    await Logging.logInfo({
      ...
      tenantID: this.tenant.id,
      action: ServerAction.OCPI_CPO_UPDATE_TARIFF,
      message: `OCPI Tariff ID '${tariffId}' has been patched successfully to IOP`,
      module: MODULE_NAME, method: 'patchTariff',
      detailedMessages: { tariffID: tariffId, newTariff, response: response.data }
    });
    return response;
  ...
  public async deleteTariff(pricing: PricingDefinition): Promise<AxiosResponse<any>> {
    const tariffId = pricing.ocpiData?.tariff?.id;
    // Check for input parameter
    if (!tariffId) {
      throw new BackendError({
        ...LoggingHelper.getPricing(pricing),
        action: ServerAction.OCPI_CPO_DELETE_TARIFF,
        module: MODULE_NAME, method: 'deleteTariff',
        message: 'Invalid parameters',
      });
    }
    // Get tariffs endpoint url
    ...
    await Logging.logInfo({
      ...
      tenantID: this.tenant.id,
      action: ServerAction.OCPI_CPO_UPDATE_TARIFF,
      message: `OCPI delete Tariff: '${tariffId}'`,
      module: MODULE_NAME, method: 'patchEVSEStatus',
      detailedMessages: { tariffID: tariffId, response: response.data }
    });
    return response;
  }
      ...
      if (!Utils.isEmptyArray(transactions.result)) {
        await Promise.map(transactions.result, async (transaction) => {
          ...
          if (transaction.stop && transaction.stop.timestamp) {
            try {
              if (await this.checkSession(transaction, tenant)) {
                result.success++;
              } else {
                result.failure++;
  ...
  public async checkLocations(): Promise<OCPIResult> {
    ...
    let next = false ;
    do {
      console.log('-----------------------check locations : skip ',skip,' limit ',limit);
      let locations = await OCPIUtilsService.getAllCpoLocations(tenants, limit, skip, options, true, this.settings);
      if (!Utils.isEmptyArray(locations.result)) {
        await Promise.map(locations.result, async (location) => {
          ...
          if (location) {
            try {
              ...
              console.log('location.id: ', location.id, 'error: ', error.message);
              result.failure++;
              result.objectIDsInFailure.push(String(location.id));
              result.logs.push(
        ...
        if (locations.count >= 25 && locations?.result) {
          next = true;
          skip = skip + 25;
        } else {
          next = false;
          console.log('--------------------------check locations done');
          break;
        }
      } else {
        next = false;
  ...
  public async checkCdrs(): Promise<OCPIResult> {
    ...
    for (const tenant of tenants) {
      ...
      if (!Utils.isEmptyArray(transactions.result)) {
        await Promise.map(transactions.result, async (transaction) => {
          try {
            if (await this.checkCdr(transaction, tenant)) {
              result.success++;
            } else {
              result.failure++;
              result.objectIDsInFailure.push(String(transaction.id));
  ...
  public async pushChargingStationStatuses(partial = false): Promise<OCPIResult> {
    ...
    // Get all locations
    const locations = await OCPIUtilsService.getAllCpoLocations(tenants, 1000, 0, options, false, this.settings);
    if (!Utils.isEmptyArray(locations.result)) {
      const totalNumberOfEvses = 0;
      for (const location of locations.result) {
        ...
        do {
          ...
          for (const tenant of tenants) {
            const findEvses = await OCPIUtilsService.getCpoEvsesFromSite(tenant, location.id, options,
              { skip: currentSkip, limit: Constants.DB_RECORD_COUNT_DEFAULT },
              { chargingStationIDs: chargeBoxIDsToProcess }, this.settings);
            if (!Utils.isEmptyArray(findEvses)) {
              evses = findEvses;
            }
          }
          // Loop through EVSE
  ...
  public async patchLocations(): Promise<OCPIResult> {
    // Result
    const result: OCPIResult = {
      success: 0,
      failure: 0,
    ...
    if (!Utils.isEmptyArray(locations.result)) {
      await Promise.map(locations.result, async (location) => {
        ...
        if (location) {
          try {
            if (await this.patchLocation(location)) {
              result.success++;
            } else {
              result.failure++;
              result.objectIDsInFailure.push(String(location.id));
            ...
            result.objectIDsInFailure.push(String(location.id));
            result.logs.push(
              `Failed to patch the Location '${location.name}' with ID '${location.id}': ${error.message as string}`
            );
          }
        }
      },
    ...
    await Logging.logOcpiResult(this.tenant.id, ServerAction.OCPI_CPO_CHECK_LOCATIONS,
      MODULE_NAME, 'checkLocations', result,
      `{{inSuccess}} Location(s) were successfully pached in ${executionDurationSecs}s`,
      `{{inError}} Location(s) failed to be patched in ${executionDurationSecs}s`,
      `{{inSuccess}} Location(s) were successfully patched and {{inError}} failed to be patched in ${executionDurationSecs}s`,
      'No Location has to be checked'
    );
    return result;
  }
  ...
  private async checkAndCreateEMSPUsersFromTokens(tokens: OCPIToken[], emspUsers: Map<string, User>, tenant: Tenant) {
    if (!Utils.isEmptyArray(tokens)) {
      for (const token of tokens) {
        ...
        if (!emspUser) {
          ...
          emspUser = await OCPIUtils.checkAndCreateEMSPUserFromToken(
            tenant, this.ocpiEndpoint.countryCode, this.ocpiEndpoint.partyId, token);
          if (emspUser) {
            // Keep in cache
  ...
  private async patchLocation(location : OCPILocation): Promise<boolean> {
    // Get locations endpoint url
    ...
    // Call IOP
    let locationTosend = JSON.stringify(location);
    const response = await this.axiosInstance.patch(
      locationUrl,
      location,
      {
    ...
    await Logging.logInfo({
      tenantID: this.tenant.id,
      action: ServerAction.OCPI_EMSP_UPDATE_LOCATION,
      message: `Location '${location.name}' with ID '${location.id}' patched successfully`,
      module: MODULE_NAME, method: 'PatchLocation',
      detailedMessages: { location }
    });
    return result;
  ...
  private async checkCdr(transaction: Transaction, childTenant: Tenant): Promise<boolean> {
    ...
    await TransactionStorage.saveTransactionOcpiData(childTenant, transaction.id, transaction.ocpiData);
    // Check CDR
    ...
    if (response.data.status_code === 3001 || response.data.status_code === 2001 || response.data.status_code === 3003) {
      await Logging.logError({
        ...LoggingHelper.getTransactionProperties(transaction),
        tenantID: this.tenant.id,
        action: ServerAction.OCPI_CPO_CHECK_CDRS,
  ...
  private async checkSession(transaction: Transaction, tenant: Tenant): Promise<boolean> {
    if (!transaction.ocpiData?.session) {
      throw new BackendError({
        ...LoggingHelper.getTransactionProperties(transaction),
  ...
  private async convertChargingStationToOCPILocation(tenant: Tenant, site: Site, chargingStation: ChargingStation,
    ...
    let chargePoint;
    if (connector) {
      connectors.push(await OCPIUtilsService.convertConnector2OcpiConnector(tenant, chargingStation, connector, countryID, partyID, this.settings));
