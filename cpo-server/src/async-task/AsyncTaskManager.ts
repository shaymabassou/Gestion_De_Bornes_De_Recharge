...
import OCPIPullTokensWithOffsetAsyncTask from './tasks/ocpi/OCPIPullTokensWithOffsetAsyncTask';
import OCPIPushEVSEStatusesAsyncTask from './tasks/ocpi/OCPIPushEVSEStatusesAsyncTask';
import OCPIPushLocationsAsyncTask from './tasks/ocpi/OCPIPushLocationsAsyncTask';
...
export default class AsyncTaskManager {
  ...
  private static async createTask(asyncTask: AsyncTask): Promise<AbstractAsyncTask> {
    ...
    switch (asyncTask.name) {
      ...
      case AsyncTasks.OCPI_PULL_TOKENS_WITH_OFFSET:
        return new OCPIPullTokensWithOffsetAsyncTask(asyncTask, correlationID);
      case AsyncTasks.OCPI_PUSH_EVSE_STATUSES:
        return new OCPIPushEVSEStatusesAsyncTask(asyncTask, correlationID);
      case AsyncTasks.OCPI_PUSH_LOCATIONS:
        return new OCPIPushLocationsAsyncTask(asyncTask, correlationID);
      default:
        await Logging.logError({
          tenantID: Constants.DEFAULT_TENANT_ID,
          action: ServerAction.ASYNC_TASK,
