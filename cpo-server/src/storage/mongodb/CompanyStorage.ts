...
import global, { DatabaseCount, Favicon, FilterParams, Logo } from '../../types/GlobalType';
...
export default class CompanyStorage {
  ...
  public static async getCompanyFavicon(tenant: Tenant, id: string): Promise<Favicon> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Read DB
    const companyFaviconMDB = await global.database.getCollection<any>(tenant.id, 'companyfavicons')
      .findOne({ _id: DatabaseUtils.convertToObjectID(id) }) as Favicon;
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'getCompanyFavicon', startTime, { id }, companyFaviconMDB);
    return {
      id: id,
      favicon: companyFaviconMDB ? companyFaviconMDB.favicon : null
    };
  }

  public static async saveCompany(tenant: Tenant, companyToSave: Company, saveLogo = true, saveFavicon = true): Promise<string> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Set
    ...
    // Save Favicon
    if (saveFavicon) {
      await CompanyStorage.saveCompanyFavicon(tenant, companyMDB._id.toString(), companyToSave.logo);
    }
  ...
  private static async saveCompanyFavicon(tenant: Tenant, companyID: string, companyFaviconToSave: string): Promise<void> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Modify
    await global.database.getCollection<any>(tenant.id, 'companyfavicons').findOneAndUpdate(
      { '_id': DatabaseUtils.convertToObjectID(companyID) },
      { $set: { favicon: companyFaviconToSave } },
      { upsert: true });
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'saveCompanyFavicon', startTime, companyFaviconToSave);
  }
}
