...
import { DataResult, OcpiEndpointDataResult } from '../../types/DataResult';
...
export default class OCPIEndpointStorage {
  ...
  public static async saveOcpiEndpoint(tenant: Tenant, ocpiEndpointToSave: OCPIEndpoint): Promise<string> {
    ...
    const ocpiEndpointMDB: any = {
      ...
      lastCPOPullTokenError : {
        url : ocpiEndpointToSave?.lastCPOPullTokenError?.url,
        dateTime:Utils.convertToDate(ocpiEndpointToSave?.lastCPOPullTokenError?.dateTime),
        response: ocpiEndpointToSave?.lastCPOPullTokenError?.response
      },
      default: ocpiEndpointToSave.default
    };
  ...
  public static async getOcpiEndpoints(tenant: Tenant,
      ...
      dbParams: DbParams, projectFields?: string[]): Promise<OcpiEndpointDataResult> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
