...
export default class TagStorage {
  ...
  public static async saveTag(tenant: Tenant, tag: Tag): Promise<void> {
    ...
    const tagMDB = {
      ...
      fromEMSP: (tag.fromEMSP) ? Utils.convertToBoolean(tag.fromEMSP) : false
    };
  ...
  public static async saveImportedTag(tenant: Tenant, importedTagToSave: ImportedTag): Promise<string> {
    ...
    const tagMDB = {
      ...
      fromEMSP: (importedTagToSave.fromEMSP) ? Utils.convertToBoolean(importedTagToSave.fromEMSP) : false
    };
  ...
  /* public static async getEmspsByTags(tenant: Tenant,
    params: {
      issuer?: boolean
    },
    dbParams: DbParams, projectFields?: string[]): Promise<DataResult<Tag>> {
    ...
    console.time('tagsCountMDB')
    // Count Records
    const tagsCountMDB = await global.database.getCollection<any>(tenant.id, 'tags')
      .aggregate([...aggregation, { $count: 'count' }], DatabaseUtils.buildAggregateOptions())
      .toArray() as DatabaseCount[];
    console.timeEnd('tagsCountMDB')
    if (!dbParams.sort) {
      dbParams.sort = { createdOn: -1 };
    }
    aggregation.push({
    ...
    if (dbParams.onlyRecordCount) {
      // Return only the count
      await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'getEmspsByTags', startTime, aggregation, tagsCountMDB);
      return {
        count: (tagsCountMDB.length > 0 ? tagsCountMDB[0].count : 0),
        result: []
      };
    ...
    // Project
    DatabaseUtils.projectFields(aggregation, projectFields);
    console.time('tagsMDB')
    // Read DB
    const tagsMDB = await global.database.getCollection<any>(tenant.id, 'tags')
      .aggregate<any>(aggregation, DatabaseUtils.buildAggregateOptions())
      .toArray() as Tag[];
    console.timeEnd('tagsMDB')
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'getEmspsByTags', startTime, aggregation, tagsMDB);
    return {
      count: DatabaseUtils.getCountFromDatabaseCount(tagsCountMDB[0]),
      result: tagsMDB,
      projectFields: projectFields
  ...
  public static async getEmspsByTags(tenant: Tenant, params: { issuer?: boolean }, dbParams: DbParams, projectFields?: string[]) {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    const tagsMDB = await global.database.getCollection<any>(tenant.id, 'tags')
      .aggregate<any>(aggregation, DatabaseUtils.buildAggregateOptions())
      .toArray() as any[];
    const fileteredTags = [...new Map(tagsMDB.map(tag => {
      let map = {
        emspName: tag._id.emspName
      }
      return [map.emspName, map]
    })).values()]
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'getEmspsByTags', startTime, aggregation, tagsMDB);
    return {
      count: fileteredTags.length,
      result: fileteredTags,
