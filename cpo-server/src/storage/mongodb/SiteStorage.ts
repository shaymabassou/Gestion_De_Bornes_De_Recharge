...
export default class SiteStorage {
  ...
  public static async saveSite(tenant: Tenant, siteToSave: Site, saveImage = false): Promise<string> {
    ...
    const siteMDB: any = {
      ...
      tariffIDs: siteToSave.tariffIDs,
    };
  ...
  public static async getSitesOCPI(tenant: Tenant, params: { issuer?: boolean; public?: boolean } = {}, projectFields?: string[]): Promise<DataResult<Site>> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Create Aggregation
    const aggregation = [];
