...
export default class TransactionStorage {
  ...
  //       $lte: Utils.convertToDate(params.ocpiCdrDateTo)
  //     }
  //     if (!params.ocpiCdrDateFrom || !moment(params.ocpiCdrDateFrom).isValid()) {
  //       delete filters['ocpiData.cdr.last_updated'].$gte
  //     }
  //     if (!params.ocpiCdrDateTo || !moment(params.ocpiCdrDateTo).isValid()) {
  //       delete filters['ocpiData.cdr.last_updated'].$lte
  //     }
  //   }
  //   /*if (params.ocpiCdrDateFrom || params.ocpiCdrDateTo) {
  //     // Start date
  ...
  //       ocpiSessionID?: string; ocpiAuthorizationID?: string; ocpiSessionDateFrom?: Date; ocpiSessionDateTo?: Date; ocpiCdrDateFrom?: Date; ocpiCdrDateTo?: Date;
  //       ocpiSessionChecked?: boolean; ocpiCdrChecked?: boolean; oicpSessionID?: string; withSite?: boolean; withSiteArea?: boolean; withCompany?: boolean;
  //       statistics?: TransactionStatisticsType; emsps?: string[];refundStatus?: RefundStatus[]; withTag?: boolean; hasUserID?: boolean; withUser?: boolean; withCar?: boolean;
  ...
  //     // add filter for emsps
  //     if (params.emsps) {
  //       aggregation.push({ '$match': { 'tag.ocpiToken.issuer': { '$in': params.emsps } } });
  //     }
  //     // TODO: [To Investigate] Cause big perf issue in prod (local it takes 2sec with this lookup instead of 165ms, in prod it can takes up to 20s)
  //     // DatabaseUtils.pushTagLookupInAggregation({
  //     //   tenantID, aggregation: aggregation, asField: 'stop.tag', localField: 'stop.tagID',
  ...
  //     $limit: dbParams.limit
  //   });
  //   // Clone aggregation filter to count transactions and statistics
  //   const countAggregation = [];
  //   for (const obj of aggregation) {
  //     if (Utils.isNullOrUndefined(obj.$limit) && Utils.isNullOrUndefined(obj.$skip)) {
  //       countAggregation.push({ ...obj });
  //     }
  //   }
  //   // set aggregation limit: if onlyRecordCount is false update limit to max
  //   if (!dbParams.onlyRecordCount) {
  //     // insert limit
  //     countAggregation.splice(3, 0, { '$limit': Constants.DB_RECORD_COUNT_CEIL });
  //   }
  ...
  public static async getTransactions(tenant: Tenant,
      params: {
        ...
        ocpiSessionID?: string; ocpiAuthorizationID?: string; ocpiSessionDateFrom?: Date; ocpiSessionDateTo?: Date; ocpiCdrDateFrom?: Date; ocpiCdrDateTo?: Date;
        ocpiSessionChecked?: boolean; ocpiCdrChecked?: boolean; oicpSessionID?: string; withSite?: boolean; withSiteArea?: boolean; withCompany?: boolean;
        statistics?: TransactionStatisticsType; refundStatus?: RefundStatus[];emsps?: string[]; withTag?: boolean; hasUserID?: boolean; withUser?: boolean; withCar?: boolean;
    ...
    if (params.ocpiAuthorizationID) {
      filters.ocpiAuthorizationID = params.ocpiAuthorizationID;
    }
  ...
  public static async getRefundReports(tenant: Tenant,
      params: { siteIDs?: string[]; userIDs?: string[]; siteAreaIDs?: string[]; ownerID?: string; siteAdminIDs?: string[]},
  ...
  public static async getTenantBySessionId(tenant: Tenant, sessionID: string,
    ...
    let childTenant;
    // search session in all tenants db
    for (const myTenant of tenants) {
      const transactionsMDB = await TransactionStorage.getTransactions(myTenant,
        {
          withUser: params.withUser,
          ocpiSessionID: sessionID
        }, Constants.DB_PARAMS_SINGLE_RECORD);

      if (transactionsMDB.count === 1) {
        childTenant = myTenant;
      }
    }
    return childTenant;
  }
