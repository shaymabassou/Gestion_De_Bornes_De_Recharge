...
import Tenant, { TenantFavicon, TenantLogo } from '../../types/Tenant';
...
export default class TenantStorage {
  public static async getTenant(id: string = Constants.UNKNOWN_OBJECT_ID,
      params: { withLogo?: boolean; withFavicon?: boolean; } = {}, projectFields?: string[]): Promise<Tenant> {
    // Call DB
  ...
  public static async saveTenant(tenantToSave: Partial<Tenant>, saveLogo = true, saveFavicon = true): Promise<string> {
    ...
    const favicon = tenantToSave.favicon;
    // Remove fields
    ...
    if (saveFavicon) {
      // Modify
      await global.database.getCollection<any>(Constants.DEFAULT_TENANT_ID, 'tenantfavicons').findOneAndUpdate(
        { '_id': tenantMDB['_id'] },
        { $set: { favicon } },
        { upsert: true });
    }
    await Logging.traceDatabaseRequestEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'saveTenant', startTime, { tenant: tenantMDB, logo: tenantMDB.logo, favicon: tenantMDB.favicon });
  ...
  public static async getTenants(
      params: { tenantIDs?: string[]; tenantName?: string; tenantSubdomain?: string; tenantType?: string; tenantParentID?: string; search?: string, withLogo?: boolean },
    ...
    if (params.tenantType) {
      filters.type = params.tenantType;
    }
    // Type
    if (params.tenantParentID) {
      filters.parentID = params.tenantParentID;
    }
    // Create Aggregation
    const aggregation = [];
    // Filters
    ...
    if (params.withLogo) {
      ...
      aggregation.push({
        $addFields: {
          ...
          favicon: {
            $concat: [
              `${Utils.buildRestServerURL()}/v1/util/tenants/favicon?ID=`,
              { $toString: '$_id' },
              {
                $ifNull: [{ $concat: ['&LastChangedOn=', { $toString: '$lastChangedOn' }] }, ''] // Only concat 'lastChangedOn' if not null
              }
  ...
  public static async deleteTenant(id: string): Promise<void> {
    ...
    // Delete favicon
    await global.database.getCollection<any>(Constants.DEFAULT_TENANT_ID, 'tenantfavicons')
      .findOneAndDelete({
        '_id': DatabaseUtils.convertToObjectID(id)
      });
  ...
  public static async getTenantFavicon(tenant: Tenant): Promise<TenantFavicon> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    let tenantFaviconMDB: { _id: ObjectId; favicon: string };
    // Read DB
    if (DatabaseUtils.isObjectID(tenant.id)) {
      tenantFaviconMDB = await global.database.getCollection<any>(Constants.DEFAULT_TENANT_ID, 'tenantfavicons')
        .findOne({ _id: DatabaseUtils.convertToObjectID(tenant.id) });
      await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'getTenantFavicon', startTime, { id: tenant.id }, tenantFaviconMDB);
    }
    return {
      id: tenant.id,
      favicon: tenantFaviconMDB?.favicon ?? null
    };
  }
}
