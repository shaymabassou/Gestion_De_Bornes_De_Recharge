...
export default class SettingStorage {
  ...
  public static async getDoubleAccessSettings(tenant: Tenant): Promise<DoubleAccessSettings> {
    const doubleAccessSettings = {
      identifier: TenantComponents.DOUBLE_ACCESS,
    } as DoubleAccessSettings;
    // Get the Ocpi settings
    const settings = await SettingStorage.getSettings(tenant,
      { identifier: TenantComponents.DOUBLE_ACCESS },
      Constants.DB_PARAMS_MAX_LIMIT);
    if (settings && settings.count > 0 && settings.result[0].content) {
      const config = settings.result[0].content;
      // ID
      doubleAccessSettings.id = settings.result[0].id;
      // double Access
      if (config.doubleAccess) {
        doubleAccessSettings.doubleAccess = config.doubleAccess;
      }
    }
    return doubleAccessSettings;
  }

  public static async getMultipleTarifsSettings(tenant: Tenant): Promise<MultipleTarifsSettings> {
    console.log('getMultipleTarifsSettings');
    const multipleTarifsSettings = {
      identifier: TenantComponents.MULTIPLE_TARIFS,
    } as MultipleTarifsSettings;
    // Get the Ocpi settings
    const settings = await SettingStorage.getSettings(tenant,
      { identifier: TenantComponents.MULTIPLE_TARIFS },
      Constants.DB_PARAMS_MAX_LIMIT);
    if (settings && settings.count > 0 && settings.result[0].content) {
      const config = settings.result[0].content;
      // ID
      multipleTarifsSettings.id = settings.result[0].id;
      // double Access
      if (config.multipleTarifs) {
        multipleTarifsSettings.isActive = Utils.convertToBoolean(config.multipleTarifs?.isActive);
      }
    }
    console.log('multipleTarifsSettings', multipleTarifsSettings);
    return multipleTarifsSettings;
  }
