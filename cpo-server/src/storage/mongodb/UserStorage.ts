...
import EmaidStorage from './EmaidStorage';
...
export default class UserStorage {
  ...
  public static async getUserByTagID(tenant: Tenant, tagID: string = Constants.UNKNOWN_STRING_ID): Promise<User> {
    let tagMDB = await TagStorage.getTag(tenant, tagID, { withUser: true });
    if(Utils.isNullOrUndefined(tagMDB)){
              console.log('--------------------here---------------------')
              tagMDB = await EmaidStorage.getEmaid(tenant,tagID)
            }
    return tagMDB ? tagMDB.user : null;
  }

  public static async getUserByEmaidID(tenant: Tenant, emaidID: string = Constants.UNKNOWN_STRING_ID): Promise<User> {
    const emaidMDB = await EmaidStorage.getEmaid(tenant, emaidID, { withUser: true });
    return emaidMDB ? emaidMDB.user : null;
  }
