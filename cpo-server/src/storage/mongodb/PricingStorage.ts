import PricingDefinition, { PricingEntity, PricingOcpiData } from '../../types/Pricing';
...
export default class PricingStorage {
  ...
  public static async savePricingDefinition(tenant: Tenant, pricingDefinition: PricingDefinition): Promise<string> {
    ...
    const pricingDefinitionMDB: any = {
      ...
      roaming: Utils.convertToBoolean(pricingDefinition.roaming)
    };

    // Save pricing ocpi data
    if (pricingDefinition.ocpiData) {
      pricingDefinitionMDB.ocpiData = {
        tariff: pricingDefinition.ocpiData.tariff,
        tariffCheckedOn: pricingDefinition.ocpiData.tariffCheckedOn,
        tariffUpdatedOn: pricingDefinition.ocpiData.tariffUpdatedOn,
        inEmsp: Utils.convertToBoolean(pricingDefinition.ocpiData.inEmsp)
      };
    }
  ...
  public static async getPricingDefinitionByName(tenant: Tenant, params: { name?: string } = {}): Promise<PricingDefinition> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Read DB
    const pricingDefinitionMDB = await global.database.getCollection<any>(tenant.id, 'pricingdefinitions')
      .findOne({ name: params.name }) as PricingDefinition;
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'pricingdefinitions', startTime, pricingDefinitionMDB);
    return pricingDefinitionMDB;
  }

  public static async getPricingDefinitions(tenant: Tenant,
      params: {roaming?: boolean, pricingDefinitionIDs?: string[], entityType?: PricingEntity; entityID?: string; siteIDs?: string[]; withEntityInformation?: boolean; dateFrom?: Date; dateTo?: Date; },
    ...
    if (params.dateFrom || params.dateTo) {
      delete filters.lastChangedOn;
      filters.lastChangedOn = {
        $gte: new Date(params.dateFrom),
        $lte: new Date(params.dateTo)
      };
      if (!params.dateFrom || !moment(params.dateFrom).isValid()) {
        delete filters.lastChangedOn.$gte;
      }
      if (!params.dateTo || !moment(params.dateTo).isValid()) {
        delete filters.lastChangedOn.$lte;
      }
    }

    if (!Utils.isNullOrUndefined(params.roaming)) {
      filters.roaming = params.roaming;
    }
  ...
  public static async getPricingDefinitionsOCPI(tenant: Tenant, params: { dateFrom?: Date; dateTo?: Date; roaming?: boolean }, projectFields?: string[]): Promise<DataResult<PricingDefinition>> {
    const uniqueTimerID = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Create Aggregation
    const aggregation = [];
  ...
  public static async savePricingOcpiData(tenant: Tenant, id: string,
      ocpiData: PricingOcpiData): Promise<void> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Modify document
    const result = await global.database.getCollection<any>(tenant.id, 'pricingdefinitions').findOneAndUpdate(
    ...
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'savePricingOcpiData', startTime, ocpiData);
  }
}
