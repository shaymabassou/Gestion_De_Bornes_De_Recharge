...
export default class LockingHelper {
  ...
  public static async acquireImportEmaidsLock(tenantID: string): Promise<Lock | null> {
    const lock = LockingManager.createExclusiveLock(tenantID, LockEntity.EMAID, 'import-emaids');
    if (!(await LockingManager.acquire(lock))) {
      return null;
    }
    return lock;
  ...
  public static async createOCPIFullPullTokensLock(tenantID: string, ocpiEndpoint: OCPIEndpoint): Promise<Lock | null> {
    return LockingHelper.acquireOCPIEndpointActionLock(tenantID, ocpiEndpoint, 'Full-pull-tokens');
  }
  ...
  public static async createOCPIPatchEVSESLock(tenantID: string, ocpiEndpoint: OCPIEndpoint): Promise<Lock | null> {
    return LockingHelper.acquireOCPIEndpointActionLock(tenantID, ocpiEndpoint, 'patch-evses');
  }
  ...
  public static async acquireBillUnpaidTransactionsLock(tenantID: string): Promise<Lock | null> {
    const lock = LockingManager.createExclusiveLock(tenantID, LockEntity.TRANSACTION, 'bill-unpaid-transactions');
    if (!(await LockingManager.acquire(lock))) {
      return null;
    }
    return lock;
  ...
  public static async acquireBillUnpaidTransactionLock(tenantID: string, transactionID: number): Promise<Lock | null> {
    const lock = LockingManager.createExclusiveLock(tenantID, LockEntity.TRANSACTION, `bill-unpaid-transaction-${transactionID}`);
    if (!(await LockingManager.acquire(lock))) {
      return null;
    }
    return lock;
